...
Module Driver
    ...
    Public g_GetDestination = "HTTG" 'g_GetDestination
    ...
    Public Function SyncVEHICLE(ByVal p_date As String, ByVal p_VEHICLE As String, ByRef o_err As String, _
        ...
        Try
            ...
            l_func = repository.CreateFunction("ZAPI_HTTG_VEHICLE")
            ...
            For p_Count = 0 To l_tabl.RowCount - 1
                ...
                'If p_Type = "H" Then                'Dong bo All
                p_SQL = "DELETE FROM tblChiTietPhuongtien where MaPhuongTien='" & l_tabl.Item(p_Count).GetValue("VEHICLE") & "'"    ' and MaNgan ='" & l_tabl_Line.Item(p_Count).GetValue("SEQ_NMBR") & "' "
                ...
                p_SQL = "MERGE tblPhuongtien AS target " & _
                        " USING (SELECT N'" & l_tabl.Item(p_Count).GetValue("VEHICLE") & "' as MaPhuongTien ," & _
                                 "N'" & l_tabl.Item(p_Count).GetValue("VEH_TYPE") & "'  as LaiXe," & _
                                ...
                                "'S' as Status) AS source (MaPhuongTien, LaiXe, SoNgan, Ngaybatdau, NgayHieuLuc, REG_NO,  Status ) " & _
                            ...
                            " WHEN MATCHED  THEN UPDATE SET " & _
                                    ...
                                    ", SyncDate= '" & p_DateSync & "',REG_NO=source.REG_NO, LaiXe = source.LaiXe " & _
        ...
        Try
            ...
            If Not p_DataTable_H Is Nothing Then
                ...
                For p_Count = 0 To p_DataTable_Line.Rows.Count - 1
                    ...
                    Double.TryParse(p_DataTable_Line.Rows(p_Count).Item("LTT_CM").ToString.Trim, p_Double)
                    If p_Double > 0 Then
                        l_tabl_Line.SetValue("LTT_CM", p_Double)
                    End If
    ...
    Public Sub KV1_UpdateTrangThaiTichKe(ByVal p_SoLenh As String, ByRef o_err As String)
        ...
        Dim p_Link, p_UserApi, p_Pass, p_SAP_API_FLAG As String
        Dim p_SQL As String
        Dim p_RowArr() As DataRow
        Dim p_SAP_API As New ECCDestination_API.JsonStructure
        ...
        Try
            ...
            Try
                ...
                p_Pass = ""
                p_SAP_API_FLAG = "N"
                p_SQL = "select * from SYS_CONFIG  where upper(KeyCode) in ('API_SAP_LINK','API_SAP_USER','API_SAP_PASS','SAP_API_FLAG') "
                p_Data = GetDataTableSQL(p_SQL, p_SQL)
                If Not p_Data Is Nothing Then
                    If p_Data.Rows.Count > 0 Then
                        p_RowArr = p_Data.Select("KeyCode='API_SAP_LINK'")
                        If p_RowArr.Length > 0 Then
                            p_Link = p_RowArr(0).Item("KeyValue").ToString.Trim
                        End If

                        p_RowArr = p_Data.Select("KeyCode='API_SAP_USER'")
                        If p_RowArr.Length > 0 Then
                            p_UserApi = p_RowArr(0).Item("KeyValue").ToString.Trim
                        End If
                        p_RowArr = p_Data.Select("KeyCode='API_SAP_PASS'")
                        If p_RowArr.Length > 0 Then
                            p_Pass = p_RowArr(0).Item("KeyValue").ToString.Trim
                        End If
                        p_RowArr = p_Data.Select("KeyCode='SAP_API_FLAG'")
                        If p_RowArr.Length > 0 Then
                            p_SAP_API_FLAG = p_RowArr(0).Item("KeyValue").ToString.Trim
                        End If
                    End If
                End If
                If p_SAP_API_FLAG = "Y" Then
                    p_SAP_API.clsUpdateTrangThaiTichKe_JSON(p_SoLenh, o_err, p_Link, p_UserApi, p_Pass)
                    Exit Sub
                End If
    ...
    Public Function Get_DO1_Infor(ByVal p_SoLenh As String, ByRef o_err As String) As DataTable
        ...
        Try
            ...
            'p_DO_Process = l_func.GetString("E_DO_PROCESS")
            'p_Row = p_TableReturn.NewRow
            'p_Row.Item("ParName") = "E_BWTAR"
            'p_Row.Item("ParValue") = p_DO_Process
            'p_TableReturn.Rows.Add(p_Row)
            '   RfcDestinationManager.UnregisterDestinationConfiguration(p_TEst)
    ...
    Private Function GetDataTable(ByVal i_Table As IRfcTable) As DataTable
    ...
    Public Function Get_SO_PO_Type_Infor(ByRef o_err As String) As DataTable
        ...
        Try
            ...
            l_func = repository.CreateFunction("ZFM_GET_PO_BSART")
            ...
            p_SQL = HistStringSyn("SO_PO_Type", True)
            p_Row = p_TableReturn.NewRow
            p_Row.Item("ParValue") = p_SQL
            p_TableReturn.Rows.Add(p_Row)
    ...
    Public Sub Post_LenhXuatBoSung(ByVal p_DataTable As DataTable, ByRef o_err As String)
        ...
        Try
            ...
            If Not p_DataTable Is Nothing Then
                ...
                l_tabl = l_func.GetTable(0) '("ZST_HTTG_PUSH_LX")
                For p_Count = 0 To p_DataTable.Rows.Count - 1
                    l_tabl.Append()
    ...
    Public Function GetDensity(ByVal p_date As String, ByRef p_TableLine As DataTable, ByRef o_err As String, _
                                ByVal p_WareHouse As String, Optional ByVal p_TankCode As String = "", Optional ByVal p_MaHangHoa As String = "") As Boolean
        Dim l_func As IRfcFunction
        Dim l_tabl As IRfcTable
        Dim l_tabl_Line As IRfcTable
        Dim l_ret2 As IRfcStructure
        ...
        Try
            ...
            If p_TableLine Is Nothing Then
                p_TableLine = New DataTable("tblDensity")
                p_TableLine.Columns.Add("TankCode")
                p_TableLine.Columns.Add("Density", GetType(System.Double))
                p_TableLine.Columns.Add("MaHangHoa")
            End If
            ...
            l_func = repository.CreateFunction("ZFM_HTTG_LIMS_GET_VCF")

            l_func.SetValue("I_DATE", p_date)
            ''20250519 
            l_func.SetValue("I_WERKS", p_WareHouse)
            If p_TankCode <> "" Then
                l_func.SetValue("I_SEQNR", p_TankCode)
            End If
            If p_MaHangHoa <> "" Then
                l_func.SetValue("I_MATNR", p_MaHangHoa)
            End If
            ...
            For p_Count = 0 To l_tabl.RowCount - 1
                ...
                ' p_Row.tab()
                p_Row = p_TableLine.NewRow
                p_Row.Item("TANKCODE") = l_tabl.Item(p_Count).GetValue("SEQNR")
                p_Row.Item("Density") = l_tabl.Item(p_Count).GetValue("GIATRI")
                p_Row.Item("MaHangHoa") = Microsoft.VisualBasic.Strings.Right(l_tabl.Item(p_Count).GetValue("MATNR").ToString, 7)
                p_TableLine.Rows.Add(p_Row)
            Next
