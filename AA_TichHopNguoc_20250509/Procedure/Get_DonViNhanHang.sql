...
-- =============================================
CREATE FUNCTION [dbo].[Get_DonViNhanHang]
(
...
BEGIN
	...
	select top 1 @p_TenDonVi=TenDonVi 
	...
	if ( @p_BSART in ('P222', 'P223')  or @p_POTYPE  in ('P222', 'P223'))    --Company code tương ứng với plant cửa hàng  (tblLenhxuatE5-Makhonhap(3) + '0')
			begin 
				...
				select top 1 @p_TenDonVi =TenDonVi  from tblDonVi  where  MaDonVi =@p_MaDonVi			
			end
	
	if ( ( @p_Inco1 ='CIF' and @p_MaNguon ='N30' ) or  (@p_MaNguon ='N05'  and @p_MaPTB in ('05','06') )  )  --NBN co TD
		begin 
			select top 1 @p_TenDonVi= TenKhachHang from  tblKhachhang where MaKhachHang = @p_MaKhachHang				
		end
