Public Class frmImfor_HoaDonVCNB_NBN
    ...
    Private p_LoaiChungTu As String = "GTGT_VCNB_NBN"
    ...
    Private Sub frmImfor_HoaDonVCNB_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        p_txtCompany = "VP " & Provider.Rows(0).Item(0).ToString()




        p_txtDonViGiaoHang = Replace(Provider_TK.Rows(0).Item(0).ToString(), "VP ", "")
        p_txtDiaChiDonViGiaoHang = Provider_TK.Rows(0).Item("DiaChiFull").ToString()

        txtDonViGiaoHang.Text = p_txtDonViGiaoHang ' Provider_TK.Rows(0).Item("TenDonVi").ToString()
        txtDiaChiDonViGiaoHang.Text = p_txtDiaChiDonViGiaoHang '  Provider_TK.Rows(0).Item("DiaChiFull").ToString()
       
        ' Provider = GetDataTable("exec InforHoaDonVCNB", p_SQL)


        ''txtCompany.Text = Provider.Rows(0).Item(0).ToString()
        'txtCompany.Text = "Cty Xăng dầu KVII - TNHH MTV"
        ''txtDonViGiaoHang.Text = Provider.Rows(0).Item(0).ToString()
        'txtDonViGiaoHang.Text = "Cty Xăng dầu KVII - TNHH MTV"
        ''txtDiaChiDonViGiaoHang.Text = Provider.Rows(0).Item(1).ToString()
        'txtDiaChiDonViGiaoHang.Text = "15 Lê Duẩn, Quận 1, Hồ Chí Minh"



        W_DSET = GetDataSet("exec sp_Reports_HoaDonE5 '" & W_SoLenh & "'", p_SQL)
        ...
        If Provider.Rows.Count > 0 Then
            ...
            p_txtDonViNhanHang = Provider.Rows(0).Item(1).ToString()

            txtDonViNhanHang.Text = Provider.Rows(0).Item(1).ToString()
            txtDiaChiDonViNhanHang.Text = Provider.Rows(0).Item(2).ToString()
            p_txtDiaChiDonViNhanHang = Provider.Rows(0).Item(2).ToString()
        End If

        If  Mid(W_DSET.Tables(0).Rows(0).Item("MaKhachHang").ToString(), Len(W_DSET.Tables(0).Rows(0).Item("MaKhachHang").ToString()) - Len("106630") + 1) = "106630" Then
            txtDonViNhanHang.Text = Provider.Rows(0).Item("TenDonVi33").ToString()
            p_txtDonViNhanHang = Provider.Rows(0).Item("TenDonVi33").ToString()
        End If



        If W_DSET.Tables(0).Rows(0).Item("ChuyenVanChuyen").ToString() = "N" _
               And Mid(W_DSET.Tables(0).Rows(0).Item("MaKhachHang").ToString(), Len(W_DSET.Tables(0).Rows(0).Item("MaKhachHang").ToString()) - Len("106630") + 1) = "106630" Then
            p_txtDonViGiaoHang = W_DSET.Tables(0).Rows(0).Item("TenDonVi22").ToString()
            p_txtDiaChiDonViGiaoHang = W_DSET.Tables(0).Rows(0).Item("DiaChiDH22").ToString()
            p_txtDiaChiDonViNhanHang = W_DSET.Tables(0).Rows(0).Item("DiaChiDH22").ToString()
        End If

        'If W_DSET.Tables(0).Rows(0).Item("ChuyenVanChuyen").ToString() = "Y" _
        '      And Mid(W_DSET.Tables(0).Rows(0).Item("MaKhachHang").ToString(), Len(W_DSET.Tables(0).Rows(0).Item("MaKhachHang").ToString()) - Len("106630") + 1) = "106630" Then
        '    p_txtDonViGiaoHang = W_DSET.Tables(0).Rows(0).Item("TenDonVi33").ToString()
        '    p_txtDiaChiDonViGiaoHang = W_DSET.Tables(0).Rows(0).Item("DiaChiDH22").ToString()
        '    p_txtDiaChiDonViNhanHang = W_DSET.Tables(0).Rows(0).Item("DiaChiDH22").ToString()
        'End If

        ' Me.dtNgayXuat.EditValue = W_DSET.Tables(0).Rows("NgayXuat")  ' p_Date
        ' Me.dtpDieuDong.EditValue = W_DSET.Tables(0).Rows("NgayXuat") 'p_Date
        ...
        Me.XtraForm1_Load()
        'hieptd4 add
        'If Not Me.dtNgayXuat.EditValue Is Nothing Then
        '    If Me.dtNgayXuat.EditValue.ToString.Trim = "" Then
        '        Me.dtNgayXuat.EditValue = W_DSET.Tables(0).Rows(0).Item("NgayXuat")
        '    End If
        'Else
        ' Me.dtNgayXuat.EditValue = W_DSET.Tables(0).Rows(0).Item("NgayXuat")
        ' End If

        'If Not Me.dtpDieuDong.EditValue Is Nothing Then
        '    If Me.dtpDieuDong.EditValue.ToString.Trim = "" Then
        ...
        If Not Me.txtDiemGiaoHang.EditValue Is Nothing Then
            If Me.txtDiemGiaoHang.EditValue.ToString.Trim = "" Then
                Me.txtDiemGiaoHang.EditValue = W_DSET.Tables(0).Rows(0).Item(19).ToString()
            End If
        Else
            Me.txtDiemGiaoHang.EditValue = W_DSET.Tables(0).Rows(0).Item(19).ToString()
        End If
        If Not Me.txtKhoNhap.EditValue Is Nothing Then
            If Me.txtKhoNhap.EditValue.ToString.Trim = "" Then
                Me.txtKhoNhap.EditValue = W_DSET.Tables(0).Rows(0).Item(20).ToString()
            End If
        Else
            Me.txtKhoNhap.EditValue = W_DSET.Tables(0).Rows(0).Item(20).ToString()
        End If
        ...
        txtDonViNhanHang.Text = p_txtDonViNhanHang
    ...
    Private Sub ToolStripButton2_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ToolStripButton2.Click
        ...
        Try
            ...
            p_RpHoaDonVCNB.Parameters.Item("P_NgayGiaoDich").Value += "/" & Format(times, "HH:mm:ss")   'String.Format("{0}:{1}", times.Hour, times.Minute)
        ...
        Try
            ...
            Select Case unitID.ToUpper
                Case "L"
                    ...
                    Full = Hed + L15 + " / " + KG + " / " + ND + " / " + D15 + " / " + VCF + " / " + WCF
                    ' Hed = "Nhiệt độ TT/L15/KG/D15/VCF/WCF: " '+ vbCrLf
                    'Full = Hed + ND + " / " + L15 + " / " + KG + " / " + D15 + " / " + VCF + " / " + WCF
                ...
                Case "KG"
                    ...
                    '    KG = Format(CInt(p_DataRow.Item("KG").ToString()), "n0").Replace(",", ".")
                    '    'Full = Hed + ND + " / " + L15 + "/" + KG + "/" + ND + " / " + D15 + " / " + VCF + " / " + WCF
                    '    Full = Hed + L15 + "/" + KG + "/" + ND + " / " + "/" + ND + " / " + D15 + " / " + VCF + " / " + WCF
