...
Module HTTG_SCADAR
    ...
    Public g_GetScadarDetail As Boolean = False
    ...
    Public Function mdlHTTG_To_Scadar(ByVal p_TypeIn As String, ByVal p_SoLenh As String, ByVal g_LoaiVanChuyen As String, _
        ...
        Try
            ...
            Catch ex As Exception
                mdlHTTG_To_Scadar = "HTTG_To_Scadar-getSQL_TableToScadar: " & ex.Message
                Exit Function
            End Try
    ...
    Private Sub UpdateScadarSTT(ByRef p_Table As DataTable)
...
Line_TT:
                p_SQL = "select dbo.FPT_GetStringDate() as StrDate"
                p_TableTmp = GetDataTable(p_SQL, p_SQL)
                If Not p_TableTmp Is Nothing Then
                    If p_TableTmp.Rows.Count > 0 Then
                        p_Str = p_TableTmp.Rows(0).Item(0).ToString.Trim
                        p_Row = p_Table.Select("Scadar_STT='" & p_Str & "'")
                        If p_Row.Length > 0 Then
                            GoTo Line_TT
                        End If

                        p_Table.Rows(p_Count).Item("Scadar_STT") = p_Str
                    End If
                End If
            Next
        Catch ex As Exception
    ...
    Private Sub SQLGetDataToScadar(ByVal p_TypeIn As String, ByVal p_SoLenh As String, ByVal g_LoaiVanChuyen As String, ByRef p_Error As Boolean, ByRef p_Desc As String, _
        ...
        Dim p_ThamSoNgay As Boolean = False
        Try
            p_ArrRow = g_Table_Sys_Config.Select("KEYCODE='THAMSONGAY'")
            If p_ArrRow.Length > 0 Then
                If p_ArrRow(0).Item("KEYVALUE").ToString.Trim = "Y" Then
                    p_ThamSoNgay = True
                End If
        ...
        If Not p_DataHTTG Is Nothing Then
            ...
            If p_DataHTTG.Columns.IndexOf("Scadar_STT") > 0 Then
                UpdateScadarSTT(p_DataHTTG)
            End If
            For p_Count = 0 To p_DataHTTG.Rows.Count - 1
                ...
                For p_CountRow = 0 To p_DataRowMap_cp.Length - 1
                    ...
                    If UCase(p_DataRowMap_cp(p_CountRow).Item("FromField").trim) = "DIACHIKHACHHANG" Then
                    ...
                    End If
                    'End If
                    If UCase(p_DataRowMap_cp(p_CountRow).Item("FromField").trim) = UCase("BeXuat") Then
                        p_SQL = p_SQL
                        ...
                        Select Case UCase(p_FieldType)
                            ...
                            Case UCase("DateTime"), UCase("Date")
                                If UCase(g_TypeConnet) = "SQL" Then
                                    ...
                                    Try
                                        If UCase(p_DataRowMap_cp(p_CountRow).Item("FromField").ToString.Trim) = "DO_CREATE" Then
                                            p_SQLValue = p_SQLValue & ",'" & p_Value & "'"     '& CDate(p_Value).ToString("yyyyMMdd  hh:mm:ss tt") & "'"
                                        Else
                                            p_SQLValue = p_SQLValue & ",'" & CDate(p_Value).ToString("yyyyMMdd") & "'"
                                        End If
                                    Catch ex As Exception
                                ...
                                End If
                            Case UCase("String")
                                p_SQLValue = p_SQLValue & ",N'" & p_Value & "'"
                        End Select
            ...
            p_TableExec.Clear()
            p_DataRow = p_TableExec.NewRow
            If g_KV1 = True Or p_ThamSoNgay = True Then
    ...
    'Truong hop phoi tron san thi ham day vao scadar thuong
    'anhqh
    'Ham thuc hien lay du lieu chi tiet cua HTTG de day sang Scadar
    Private Sub SQLGetDataToScadar_InTank(ByVal p_TypeIn As String, ByVal p_SoLenh As String, ByVal g_LoaiVanChuyen As String,
