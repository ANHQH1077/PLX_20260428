...
Imports System.IO
Module SAP_HTTG
    Public p_FunctionTableId As String
    ...
    Public p_ThamSoNgay As Boolean = False
                            ...
                            If Not p_Table Is Nothing Then
                                If p_Table.Rows.Count > 0 Then
                                    If p_Table.Rows(0).Item("HoVaTen").ToString.Trim <> "" Then
                                        p_SQL = "UPDATE tbllenhxuate5_THN set  NguoiVanChuyen =N'" & p_Table.Rows(0).Item("HoVaTen").ToString.Trim & _
    ...
    Private Function mdlSyncDeliveries_SubModifyFromTable_new(ByVal p_Client As String, _
        ...
        l_sql = "select Solenh, Status from tblLenhXuatE5_THN with (Nolock)   where SoLenh = '" & i_lt_temp.Item(0).Outbound.ToString() & "'"
        l_err = String.Empty
...
TaoMaLenh:
        ...
        If p_DataExec.Rows.Count > 0 Then
            ...
            p_DataRow.Item(0) = "DELETE FROM tblLenhXuat_HangHoaE5_THN where SoLenh='" & l_wa.Outbound.ToString() & "'"
        ...
        If p_DataExec.Rows.Count > 0 Then
            p_DataRow = p_DataExec.NewRow
            p_DataRow.Item(0) = "UPDATE tblLenhXuatE5_THN set NhaCungCap='" & p_NCC & "' where SoLenh='" & l_wa.Outbound.ToString() & "'"
            p_DataExec.Rows.Add(p_DataRow)
        End If
        ...
        p_SQL = "select TableID, MaHangHoa, (select top 1  TenHangHoa from  tblHangHoa where MaHangHoa =a.MaHangHoa) as TenHangHoa  ,DonViTinh,TongDuXuat,TongXuat, MeterID, BeXuat, SoLenh,MaLenh," & _
               "NgayXuat,LineID  from tblLenhXuat_hanghoaE5_THN a where solenh='" & l_wa.Order_No & "'"
            ...
            If p_TableHangHoa.Rows.Count > 0 Then
                ...
                If Not p_TableChiaNgan Is Nothing Then
                    ...
                    For p_Count = 0 To p_TableChiaNgan.Rows.Count - 1
                        ...
                        p_SQL = "INSERT INTO tblLenhXuatChiTietE5_THN (DungTichNgan,MaNgan , MaLenh ,NgayXuat , LineID , SoLuongDuXuat , TableID , MaTuDongHoa, SoLenhLine )"
        ...
        If l_loaiphieu <> "V144" And p_DUXUAT_TD = True Then
            ...
            If p_TableHangHoa.Rows.Count > 0 Then
                For p_Count = 0 To p_TableHangHoa.Rows.Count - 1
                    p_DataRow = p_DataExec.NewRow
                    p_DataRow.Item(0) = "update  tblLenhXuat_HangHoaE5_THN  set TongXuat  =" & IIf(p_TableHangHoa.Rows(p_Count).Item("TongXuat").ToString.Trim = "", 0, p_TableHangHoa.Rows(p_Count).Item("TongXuat").ToString.Trim) & ",  " & _
                            ...
                            " where LineID='" & p_TableHangHoa.Rows(p_Count).Item("LineID").ToString.Trim & "' " & _
...
TaoMaLenh:
        ...
        'Dim p_Row As System.Data.DataRow
        p_SQL = "select TableID, MaHangHoa, (select top 1  TenHangHoa from  tblHangHoa where MaHangHoa =a.MaHangHoa) as TenHangHoa  ,DonViTinh,TongDuXuat,TongXuat, MeterID, BeXuat, SoLenh,MaLenh," & _
               "NgayXuat,LineID  from tblLenhXuat_hanghoaE5_THN where solenh='" & l_wa.Order_No & "'"
        p_TableHangHoa = GetDataTable(p_SQL, p_SQL)
        p_TableHangHoa.Columns.Add("NhietDo")
        p_TableHangHoa.Columns.Add("TaiTrong")
        p_TableHangHoa.Columns.Add("TyTrong")
    ...
    Private Function mdlSyncDeliveries_SubModifyFromWorkAreaLine(ByRef p_DataExec As DataTable, ByVal i_wa As Connect2SAP.Str_PhieuXuat, _
        ...
        Try
            ...
            p_SQL = "INSERT INTO [tblLenhXuat_HangHoaE5_THN] (LineID, MaLenh,NgayXuat, SoLenh,TongXuat, " & _
    ...
    Private Function mdlSyncDeliveries_SubModifyFromWorkAreaLine_QCI(ByVal p_Client As String, ByVal l_ztb2 As Connect2SapEx.LIPSO2Table, _
        ...
        Try
            ...
            If p_CONGTO_BEXUAT = True And p_BeXuat.ToString.Trim <> "" Then
                ...
                p_SQL = "INSERT INTO [tblLenhXuat_HangHoaE5_THN] (NhomBeXuat,BatchNum, ChietKhau, MeterId, LineID, MaLenh,NgayXuat, SoLenh,TongXuat, " & _
            ...
            Else
                p_SQL = "INSERT INTO [tblLenhXuat_HangHoaE5_THN] (NhomBeXuat, BatchNum,ChietKhau, LineID, MaLenh,NgayXuat, SoLenh,TongXuat, " & _
                     "TongDuXuat, MaHangHoa, DonViTinh,BeXuat,TableID ,Createby,CreateDate, QCI_KG, QCI_NhietDo, QCI_TyTrong) "
    ...
    Private Sub KV1_GenScriptfromTbl(ByVal p_Client As String, ByVal l_ztb2 As Connect2SapEx.LIPSO2Table, _
        ...
        If p_TableHangHoa.Rows.Count > 0 Then
            ...
            For p_Count = 0 To p_TableHangHoa.Rows.Count - 1
                ...
                p_SQL = "INSERT INTO [tblLenhXuat_HangHoaE5_THN] (ChietKhau, LineID, MaLenh,NgayXuat, SoLenh,TongXuat, " & _
                    "TongDuXuat, MaHangHoa, DonViTinh,BeXuat,TableID ,Createby,CreateDate, QCI_KG, QCI_NhietDo, QCI_TyTrong) "
    ...
    Public Function GetTableID() As String
        Dim p_SQL As String
        Dim p_DataTable As DataTable
    ...
    'anhqh 20160920
    Public Function GetTableIDKV1(ByVal p_MaVanChuyen As String, ByVal p_date As Date, ByVal p_HangHoa As String) As String
        Dim p_SQL As String
        Dim p_DataTable As DataTable
        ...
        Try
            ...
            If Check_SoLenh(l_err, l_solenh) = False Then   'Chua ton tai so lenh
                If NgayHetHieuLuc <> CDate("01/01/1900") Then
                    p_SQL = "INSERT INTO [tblLenhXuatE5_THN] (NhomBeXuat,so_tktx, NOTE_SMO, SO_CHUYEN, LOAI_KH, BatchNum, DO1_MaNguon,CreateDate, PriceGroupDO1, SOType, PrcingDate, Inco1, Ngay_TKTX , DO1_SoLenh, DO1_MaKhach,STO, DiemTraHang, NgayHetHieuLuc, Slog, Client, SoLenhSAP,[MaLenh],[NgayXuat],[SoLenh],[MaDonVi],[MaNguon],[MaKho]," &
            ...
            Else  ''Da ton tai so lenh
                p_SQL = "UPDATE [dbo].[tblLenhXuatE5_THN] SET  NhomBeXuat= '" & g_NhomBeXuat & "', so_tktx = '" & p_SoTKTX & "', DO1_MaNguon='" & p_MaNguonDo1 & "', PriceGroupDO1='" & p_KONDA & "', DO1_SoLenh ='" & p_SoLenh_Do1 & "', DO1_MaKhach ='" & p_MaKhachDo1 & "', STO='" & l_STO & "', DiemTraHang =N'" & p_DiemTraHang & "', NgayHetHieuLuc='" & CDate(NgayHetHieuLuc).ToString("yyyyMMdd") & "', Slog='" & p_Slog & "', Client='" & p_Client & "',MaDonVi = '" & l_madonvi & "',MaNguon = '" & l_manguon & "', MaKho ='" & l_makho & "'" &
    ...
    Public Function mdlSyncDeliveries_SynSpecific_ExNew(ByVal p_Client As String, ByVal p_User_ID As Integer, _
        ...
        p_SQL = "select DO1_SoLenh from tblLenhXuatE5_THN where SoLenh ='" & i_solenh & "'"
        p_Table = GetDataTable(p_SQL, p_SQL)
        If Not p_Table Is Nothing Then
            If p_Table.Rows.Count > 0 Then
        ...
        Try
            ...
            p_SQL = "update   tblDO_Item_Material  set  LineID  = (select top 1 LineID from tbllenhxuat_HangHOae5_THN  k " & _
