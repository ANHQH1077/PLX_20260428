Public Class frmBeATG
    ...
    Public Sub Column_Button_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        If p_Row Is Nothing Then
            Exit Sub
        End If
        p_TankName = p_Row.Item("TankCode").ToString.Trim
        If p_TankName = "" Then
            ShowMessageBox("", "Bể không xác định")
            Exit Sub
        End If
        p_ValueMess = p_XtraModuleObj.ShowMessage(Me, "", _
                                        "Bạn có chắn chắn muốn đồng bộ Barem bể " & p_TankName & " không?", _
                                        True, _
                                         "Có", _
                                        True, _
                                        "Không", _
        ...
        Dim p_SyncMaster As New SynMaster.Class1(g_WCF, g_Services, flag, lw_mess, "H", _
                  g_dt, _SapConnectionString, _WareHouse, _dtVariable, g_Company_Code, _ShPoint, g_UserName)
        If p_SyncMaster.clsSyncMaster_SyncTank_Barem(p_DataExec, "A", l_date, _WareHouse, p_Desc, l_Time, p_TankName) = True Then
            ShowMessageBox("", "Đã thực hiện xong", 1)
        Else
            ShowMessageBox("", p_Desc)
        End If
