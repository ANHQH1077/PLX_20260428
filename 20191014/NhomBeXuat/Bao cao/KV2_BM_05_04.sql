...
BEGIN
	...
	--select @p_FromDateTime, @p_ToDateTime
	-- return
...
print @p_todatetime
	...
	select MaHangHoa, 
			...
			, isnull(a1.NhomBe,'') as NhomBe 
				, case when isnull(a1.NhomBe,'')  <> '' then  isnull(a1.NhomBe,'')  + ' - ' +  a2.GrpName else '' end as TenNhomBe
		from  #TableVarTmp  a1 
   ...
   union  all
	...
	select MaHangHoa, 
				...
				0 as gst
				, a2.Code as NhomBe 
				, case when isnull(a1.NhomBeXuat,'')  <> '' then  isnull(a1.NhomBeXuat,'')  + ' - ' +  a2.GrpName else '' end as TenNhomBe
			from  @TableVar22 	a1  
