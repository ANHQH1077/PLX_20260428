...
ALTER VIEW [dbo].[FPT_tblLenhXuatE5_New_V]
AS
SELECT     A.MaLenh, A.NgayXuat, A.SoLenh, A.MaDonVi, A.MaNguon,
                         ...
                         (select TenKho from dbo.tblKho AS h1 WITH (Nolock) where h1.makho =a.MaKho ) 
                           AS TenKhoXuat
                         -- ,'' as TenKho
                         ,case when 	isnull(a.TenKhoNhap ,'') ='' then h.TenKho else  a.TenKhoNhap end TenKho
                          , A.MaVanChuyen,
                      ...
                      A.User_Approve, A.Date_Approve, A.EditApprove, A.NhaCungCap,
                          ...
                          (SELECT     TOP (1) SoTichKe
                            ...
                            WHERE      (SoLenh = A.SoLenh)) AS SoTichKe, A.AppDesc, A.Slog, A.NgayHetHieuLuc, A.NgayTichKe, A.STO, A.NguoiDaiDien, b.TenPhuongThucXuat, A.DiemTraHang, c.TenNhaCungCap, A.Tax, 
                      A.PaymentMethod, d.NoiDung AS PhuongThucThanhToan, A.Term, e.GhiChu AS ThoiHanThanhToan, A.MaTuyenDuong, g.DGTuyenDuong AS TenTuyenDuong, A.MaKhoNhap, 
                      
                      --h.TenKho AS TenKhoNhap,
                      a.makhonhap + ' - ' + 
					  case when 	isnull(a.TenKhoNhap ,'') ='' then h.TenKho else  a.TenKhoNhap end AS TenKhoNhap,
                      
                       A.SoHopDong, A.NgayHopDong, A.TyGia, A.SoTKHQNhap, A.SoTKHQXuat, A.SelfShipping, A.PriceGroup, pg.PriceGroupName, A.Inco1, A.Inco2, A.XuatHangGui, A.So_TKTN, 
                      A.So_TKTX, A.Ngay_TKTX
                      , DischargePoint , DesDischargePoint, a.BSART, b.Ma_Map
                      ,g.DGDiemDen,
                     
                      b.Ma_map + '-' + b.BWART + '-'+  b.TenPhuongThucXuat  as TenPhuongThucXuat1
                      , a.TenKhoNhap as TenKhoNhap11
FROM         dbo.tblLenhXuatE5 AS A WITH (Nolock) LEFT OUTER JOIN
                      dbo.tblPhuongThucXuat AS b WITH (Nolock) ON b.BWART = A.MaPhuongThucXuat 
