...
-- =============================================
alter PROCEDURE FPT_SyncFPT_SyncSYS_TABLEID_NEW
AS
BEGIN
	...
	DECLARE SoLenh_cursor CURSOR FOR   
		select ID, CrdDate, TableID, Old_ID  from SERVER_HTTG.[HTTG_UAT].[dbo].[SYS_TABLEID_NEW_Hist] where isnull( SynStatus,'') = ''  
	...
	WHILE @@FETCH_STATUS = 0  
		BEGIN  
			BEGIN TRY 			
				begin	
					 INSERT INTO  [SYS_TABLEID_NEW_TMP]
								   ([TableID]
							   ,[CrdDate]
							   ,[SoLenh], ID)
					 select 	[TableID]
							   ,[CrdDate]
							   ,[SoLenh], ID
					from  SERVER_HTTG.[HTTG_UAT].[dbo].SYS_TABLEID_NEW
					
						where 	  ID= @p_Old_ID 		
				
					update SERVER_HTTG.[HTTG_UAT].[dbo].[SYS_TABLEID_NEW_Hist] set SynDate=getdate(), SynStatus='Y' where ID=@p_ID	--and CrdDate=@p_NgayXuat
				end
			END TRY  
			BEGIN CATCH 
