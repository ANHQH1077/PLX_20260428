...
-- =============================================
alter PROCEDURE FPT_SyncSYS_TABLEID_ZR_Hist99	
AS
BEGIN
	...
	DECLARE SoLenh_cursor CURSOR FOR   
		select ID, CrdDate from SERVER_HTTG.[HTTG_UAT].[dbo].[SYS_TABLEID_ZR_Hist99] where isnull( SynStatus,'') = ''  
		...
		BEGIN  
			BEGIN TRY 				
			   if isnull(@p_cStatus,'')='D'
					begin
						delete from  [SYS_TABLEID_ZR_Hist]
								where ID=@p_ID 										
										and CrdDate= @p_NgayXuat
										
						update SERVER_HTTG.[HTTG_UAT].[dbo].[SYS_TABLEID_ZR_Hist99] set SynDate=getdate(), SynStatus='Y' where ID=@p_ID	 and CrdDate= @p_NgayXuat					
					end
				else --if isnull(@p_cStatus,'')='U'					
						begin	
							 INSERT INTO  [SYS_TABLEID_ZR_Hist]
										   ([TableID]
										   ,[CrdDate]
										   ,[SoLenh]
										   ,[MaHangHoa]
							 ...
							 select 	[TableID]
										   ...
										   ,[MaHangHoa]
										   ,[ID]
							from  SERVER_HTTG.[HTTG_UAT].[dbo].SYS_TABLEID_ZR_Hist
								where 	ID=@p_ID 										
										and CrdDate= @p_NgayXuat
							 update SERVER_HTTG.[HTTG_UAT].[dbo].[SYS_TABLEID_ZR_Hist99] set SynDate=getdate(), SynStatus='Y' where ID=@p_ID and CrdDate= @p_NgayXuat
						end
