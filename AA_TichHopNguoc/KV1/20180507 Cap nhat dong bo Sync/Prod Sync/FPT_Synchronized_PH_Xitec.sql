...
BEGIN
	...
	DECLARE SoLenh_cursor CURSOR FOR   
		...
		select ID_LenhXuat, 'Y' as cStatus , Ma_Lenh, Ma_Ngan,Ngay_ctu, ID_LenhXuat from SERVER_HTTG.[HTTG_UAT].[dbo].PH_Xitec a with (nolock)
			
			where  convert(date,Ngay_ctu)>=convert(date,getdate()-1)			
				and  not exists 
									(select 1 from PH_Xitec  b   with (nolock) where 
		...
		BEGIN  
			BEGIN TRY 				
			   if isnull(@p_cStatus,'')='D'
					...
					begin
						IF EXISTS(SELECT 1 FROM [PH_Xitec] with (nolock)
									where Ma_Lenh=@p_MaLenh
										and Ma_Ngan=@p_MaNgan
										and Ngay_ctu= @p_NgayXuat)
							begin
								 UPDATE sr  
										...
										FROM [PH_Xitec] AS sr  
											inner JOIN SERVER_HTTG.[HTTG_UAT].[dbo].[PH_Xitec]  AS source   
							...
							begin	
								 ...
								 select 	[SoLenh] ,[LineID] ,[Ma_lenh] ,[Ngay_ctu],[Ma_ngan],[Ma_hhoa]
										   ,[So_ptien],[Sl_preset],[Sl_ltt],[Nhiet_do],[Thoigiandau]
										   ,[Thoigiancuoi],[Ma_hong],[Trang_thai],[sl_llkebd],[sl_llkekt]
									FROM  SERVER_HTTG.[HTTG_UAT].[dbo].PH_Xitec
