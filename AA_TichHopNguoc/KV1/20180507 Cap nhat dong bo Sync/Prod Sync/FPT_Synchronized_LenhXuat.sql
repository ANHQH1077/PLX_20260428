...
-- =============================================
alter PROCEDURE FPT_Synchronized_LenhXuat		
AS
BEGIN
	...
	DECLARE SoLenh_cursor CURSOR FOR   
		select ID, SoLenh, cStatus from SERVER_HTTG.[HTTG_UAT].[dbo].[tbllenhxuate5_Hist] with (nolock) where isnull( SynStatus,'') = ''  
		...
		BEGIN  
			BEGIN TRY 				
			   if isnull(@p_cStatus,'')='D'
					begin
						delete from  [tblLenhXuatE5]
								where Solenh=@p_SoLenh
						update SERVER_HTTG.[HTTG_UAT].[dbo].[tbllenhxuate5_Hist] set SynDate=getdate(), SynStatus='Y' where ID=@p_ID
						
					end
				else --if isnull(@p_cStatus,'')='U'
					begin
						IF EXISTS(SELECT 1 FROM [tblLenhXuatE5] with (nolock)
								  WHERE  SoLenh =@p_SoLenh)
							begin
								...
								UPDATE sr  
										set [MaLenh]=source.[MaLenh],[NgayXuat]=source.[NgayXuat],[SoLenh]=source.[SoLenh]
										  ...
										  ,[TenKhoNhap]=source.[TenKhoNhap] ,[Xitec_option]=source.[Xitec_option] ,[SoLenhGoc]=source.[SoLenhGoc] 
										   ,SynDate =getdate()
											, SynStatus ='Y'
										FROM [tblLenhXuatE5] AS sr  
										inner JOIN SERVER_HTTG.[HTTG_UAT].[dbo].[tblLenhXuatE5]  AS source   
							...
							--print ('Insert')
							   ...
							   select  [MaLenh],[NgayXuat],[SoLenh],[MaDonVi] ,[MaNguon],[MaKho],[MaVanChuyen]
								...
								from SERVER_HTTG.[HTTG_UAT].[dbo].tblLenhXuatE5 with (nolock) where SoLenh=@p_SoLenh
