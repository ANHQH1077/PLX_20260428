...
BEGIN
	...
	DECLARE SoLenh_cursor2 CURSOR FOR   
		select Row_ID, ID, cStatus, TableID, LineID from SERVER_HTTG.[HTTG_UAT].[dbo].[tblLenhXuatChiTietE5_Hist] with (nolock) where isnull( SynStatus,'') = '' 
			---and SoLenh=@p_SoLenh  
		...
		BEGIN  
			BEGIN TRY 				
			   if isnull(@p_cStatus,'')='D'
					begin
						delete from  [tblLenhXuatChiTietE5]
								where MapRowID =@p_Row_ID
						 update SERVER_HTTG.[HTTG_UAT].[dbo].[tblLenhXuatChiTietE5_Hist] set SynDate=getdate(), SynStatus='Y' where 
										ID=@p_ID
					end
				else --if isnull(@p_cStatus,'')='U'
					begin
						IF EXISTS(SELECT 1 FROM [tblLenhXuatChiTietE5] with (nolock)
								  WHERE   MapRowID =@p_Row_ID)
							begin
								 UPDATE sr
							...
							FROM [tblLenhXuatChiTietE5] AS sr  
										inner JOIN SERVER_HTTG.[HTTG_UAT].[dbo].[tblLenhXuatChiTietE5]  AS source   
							...
							begin
							     INSERT INTO [tblLenhXuatChiTietE5]
								...
								from SERVER_HTTG.[HTTG_UAT].[dbo].[tblLenhXuatChiTietE5]  with (nolock)
