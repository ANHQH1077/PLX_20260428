...
-- =============================================
alter PROCEDURE FPT_Synchronized_LenhXuatHangHoa
...
BEGIN
	...
	DECLARE SoLenh_cursor1 CURSOR FOR   
		select  ID,  SoLenh, cStatus, TableID, LineID, MaHangHoa from SERVER_HTTG.[HTTG_UAT].[dbo].[tblLenhXuat_HangHoaE5_Hist] with (nolock) 
			where isnull( SynStatus,'') = '' 
			---and SoLenh=@p_SoLenh  
		...
		BEGIN  
			BEGIN TRY 				
			   if isnull(@p_cStatus,'')='D'
					begin
						delete from  [tblLenhXuat_HangHoaE5]
								where Solenh=@p_SoLenh
									 and TableID = @p_TableID 
											 and LineID = @p_LineID
											 and MaHangHoa =  @p_MaHangHoa
						update SERVER_HTTG.[HTTG_UAT].[dbo].[tblLenhXuat_HangHoaE5_Hist] set SynDate=getdate(), SynStatus='Y' where 
										ID=@p_ID
					end
				else --if isnull(@p_cStatus,'')='U'
					begin
						IF EXISTS(SELECT 1 FROM [tblLenhXuat_HangHoaE5] with (nolock)
								 ...
								 UPDATE sr  
										set [MaLenh] = source.MaLenh
											  ...
											  ,[TheBVMT] = source.TheBVMT
											   ,SynDate =getdate()
												, SynStatus='Y'
										FROM [tblLenhXuat_HangHoaE5] AS sr   with (nolock)
										inner JOIN SERVER_HTTG.[HTTG_UAT].[dbo].[tblLenhXuat_HangHoaE5]  AS source    
							...
							begin
							     INSERT INTO [tblLenhXuat_HangHoaE5]
								 ...
								 FROM SERVER_HTTG.[HTTG_UAT].[dbo].tblLenhXuat_HangHoaE5 with (nolock) 
