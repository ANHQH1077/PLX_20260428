...
-- =============================================
alter PROCEDURE FPT_SyncSYS_TABLEID
AS
BEGIN
	...
	DECLARE SoLenh_cursor CURSOR FOR   
		select ID, CrdDate, TableID, Old_ID from SERVER_HTTG.[HTTG_UAT].[dbo].[SYS_TABLEID_Hist] where isnull( SynStatus,'') = ''  
		...
		BEGIN  
			BEGIN TRY 				
			   if isnull(@p_cStatus,'')='D'
					begin
						delete from  [SYS_TABLEID]
								where  Old_ID=@p_Old_ID
								--TableID=@p_TableID 										
								--		and CrdDate= @p_NgayXuat										
						update SERVER_HTTG.[HTTG_UAT].[dbo].[SYS_TABLEID_Hist] set SynDate=getdate(), SynStatus='Y' where ID=@p_ID	
					end
				else --if isnull(@p_cStatus,'')='U'			
					 IF EXISTS(SELECT 1 FROM [SYS_TABLEID] with (nolock)
									where Old_ID=@p_Old_ID)
						begin
								update 	[SYS_TABLEID]
							...
							begin	
									 ...
									 select 	[TableID]
											   ...
											   ,[SoLenh]
											   , ID
									from  SERVER_HTTG.[HTTG_UAT].[dbo].SYS_TABLEID
										where 	  ID= @p_Old_ID				
