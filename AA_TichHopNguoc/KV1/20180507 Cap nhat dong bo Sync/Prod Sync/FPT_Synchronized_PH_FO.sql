...
-- =============================================
alter PROCEDURE FPT_Synchronized_PH_FO	
AS
BEGIN
	...
	DECLARE SoLenh_cursor CURSOR FOR   
		--select ID, cStatus , Ma_Lenh, Ma_Ngan,Ngay_ctu, ID_LenhXuat from [PH_FO_HIST] where isnull( SynStatus,'') = ''  
		--	order by  isnull(UpdateDate,Createdate) asc, ID asc
		select ID_LenhXuat, 'Y' as cStatus , Ma_Lenh, Ma_Ngan,Ngay_ctu, ID_LenhXuat from SERVER_HTTG.[HTTG_UAT].[dbo].PH_FO a with (nolock)
			
			where  convert(date,Ngay_ctu)>=convert(date,getdate()-1)			
				and  not exists 
									(select 1 from PH_FO  b   with (nolock) where 
			...
			BEGIN TRY 				
			   if isnull(@p_cStatus,'')='D'
					begin
						...
						--update [PH_FO_HIST] set SynDate=getdate(), SynStatus='Y' where ID=@p_ID
						
					end
				else --if isnull(@p_cStatus,'')='U'
					begin
						IF EXISTS(SELECT 1 FROM [PH_FO] with (nolock)
									where Ma_Lenh=@p_MaLenh
										and Ma_Ngan=@p_MaNgan
										and Ngay_ctu= @p_NgayXuat)
							begin
								 UPDATE sr  
										...
										FROM [PH_FO] AS sr  
											inner JOIN SERVER_HTTG.[HTTG_UAT].[dbo].[PH_FO]  AS source   
							...
							begin	
								 ...
								 select 	[SoLenh] ,[LineID] ,[Ma_lenh] ,[Ngay_ctu],[Ma_ngan],[Ma_hhoa]
										   ,[So_ptien],[Sl_preset],[Sl_ltt],[Nhiet_do],[Thoigiandau]
										   ,[Thoigiancuoi],[Ma_hong],[Trang_thai],[sl_llkebd],[sl_llkekt]
									FROM  SERVER_HTTG.[HTTG_UAT].[dbo].PH_FO
