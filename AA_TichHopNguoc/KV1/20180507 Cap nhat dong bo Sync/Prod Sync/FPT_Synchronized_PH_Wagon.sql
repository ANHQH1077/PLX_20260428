...
-- =============================================
alter PROCEDURE FPT_Synchronized_PH_Wagon	
AS
BEGIN
	...
	DECLARE SoLenh_cursor CURSOR FOR   
		--select ID, cStatus , Ma_Lenh, Ma_Ngan,Ngay_ctu, ID_LenhXuat from [PH_Wagon_HIST] where isnull( SynStatus,'') = ''  
		--	order by  isnull(UpdateDate,Createdate) asc, ID asc
		select ID_LenhXuat, 'Y' as cStatus , Ma_Lenh, Ma_Ngan,Ngay_ctu, ID_LenhXuat from SERVER_HTTG.[HTTG_UAT].[dbo].PH_Wagon a with (nolock)
			
			where  convert(date,Ngay_ctu)>=convert(date,getdate()-1)			
				and  not exists 
									(select 1 from PH_Wagon  b   with (nolock) where 
										 a.Ngay_ctu=b.Ngay_Ctu
										and a.Ma_Lenh=b.Ma_Lenh
										and  a.Ma_Ngan=b.Ma_Ngan
										and a.trang_thai =b.trang_thai
									)
				order by  Ngay_ctu asc, a.ID_LenhXuat asc
	OPEN SoLenh_cursor  
		...
		BEGIN  
			BEGIN TRY 				
			   if isnull(@p_cStatus,'')='D'
					begin
						delete from  [PH_Wagon]
								where Ma_Lenh=@p_MaLenh
										and Ma_Ngan=@p_MaNgan
										and Ngay_ctu= @p_NgayXuat
										
						--update [PH_Wagon_HIST] set SynDate=getdate(), SynStatus='Y' where ID=@p_ID
						
					end
				else --if isnull(@p_cStatus,'')='U'
					begin
						IF EXISTS(SELECT 1 FROM [PH_Wagon] with (nolock)
									where Ma_Lenh=@p_MaLenh
										and Ma_Ngan=@p_MaNgan
										and Ngay_ctu= @p_NgayXuat)
							begin
								 UPDATE sr  
										...
										FROM [PH_Wagon] AS sr  
											inner JOIN SERVER_HTTG.[HTTG_UAT].[dbo].[PH_Wagon]  AS source   
							...
							begin	
								 ...
								 select 	[SoLenh] ,[LineID] ,[Ma_lenh] ,[Ngay_ctu],[Ma_ngan],[Ma_hhoa]
										   ,[So_ptien],[Sl_preset],[Sl_ltt],[Nhiet_do],[Thoigiandau]
										   ,[Thoigiancuoi],[Ma_hong],[Trang_thai],[sl_llkebd],[sl_llkekt]
									FROM  SERVER_HTTG.[HTTG_UAT].[dbo].PH_Wagon
