...
-- =============================================
Create PROCEDURE FPT_Synchronized_PH_FO	
AS
BEGIN
		...
		BEGIN  
			BEGIN TRY 				
			   if isnull(@p_cStatus,'')='D'
					begin
						delete from  LOCALSERVER.[HTTG_UAT].[dbo].[PH_FO]
					...
					begin
						IF EXISTS(SELECT 1 FROM LOCALSERVER.[HTTG_UAT].[dbo].[PH_FO] with (nolock)
									where Ma_Lenh=@p_MaLenh
										and Ma_Ngan=@p_MaNgan
										and Ngay_ctu= @p_NgayXuat)
							begin
								 UPDATE sr  
										...
										FROM LOCALSERVER.[HTTG_UAT].[dbo].[PH_FO] AS sr  
											inner JOIN [PH_FO]  AS source   
							...
							begin	
								 INSERT INTO LOCALSERVER.[HTTG_UAT].[dbo].[PH_FO]
