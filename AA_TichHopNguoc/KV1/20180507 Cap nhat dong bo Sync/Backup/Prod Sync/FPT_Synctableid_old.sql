...
-- =============================================
create PROCEDURE FPT_SyncFPT_Synctableid_old
AS
BEGIN
	...
	DECLARE SoLenh_cursor CURSOR FOR   
		select ID, NgayXuat,MaxTable from [Sys_tableid_old_Hist] where isnull( SynStatus,'') = ''  
		...
		BEGIN  
			BEGIN TRY 				
			   if isnull(@p_cStatus,'')='D'
					begin
						delete from  LOCALSERVER.[HTTG_UAT].[dbo].[Sys_tableid_old]
								where MaxTable=@p_MaxTable 										
										and NgayXuat= @p_NgayXuat										
						update [Sys_tableid_old_Hist] set SynDate=getdate(), SynStatus='Y' where ID=@p_ID	 and NgayXuat= @p_NgayXuat
														and MaxTable =@p_MaxTable					
					end
				else --if isnull(@p_cStatus,'')='U'					
						begin	
							 INSERT INTO  LOCALSERVER.[HTTG_UAT].[dbo].[Sys_tableid_old]
										   ([SoLenh]
											   ,[MaxTable]
											   ,[NgayXuat])
							 select 	[SoLenh]
