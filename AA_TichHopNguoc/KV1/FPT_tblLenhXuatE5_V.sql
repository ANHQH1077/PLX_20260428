...
create VIEW [dbo].[FPT_tblLenhXuatE5_V]
AS
SELECT     A.MaLenh, A.NgayXuat, A.SoLenh, A.MaDonVi, A.MaNguon,
                          ...
                          (SELECT     TOP (1) TenKhachHang
                            ...
                            WHERE      (MaKhachHang = A.MaKhachHang)) AS TenKhachHang,
                            (SELECT     TOP (1) TenVietTat
                            FROM          dbo.tblKhachHang AS tblKhachHang_1
                            WHERE      (MaKhachHang = A.MaKhachHang)) AS TenVietTat,
                             A.LoaiPhieu, A.Niem, A.LuongGiamDinh, A.NhietDoTaiTau, A.GhiChu, A.NgayHieuLuc, 
                      RTRIM(LTRIM(A.Status)) AS Status, A.Number, A.Createby, A.UpdatedBy, A.CreateDate, A.UpdateDate, A.SoLenhSAP, 
                      CASE WHEN LoaiPhieu = 'V144' THEN 'N' ELSE 'Y' END AS ChuyenVanChuyen, A.Client, A.HTTG, A.Approved, A.User_Approve, A.Date_Approve, A.EditApprove, 
                      A.NhaCungCap,
                          (SELECT     Name
                          ...
                          (SELECT     TOP (1) SoTichKe
                            ...
                            WHERE      (SoLenh = A.SoLenh)) AS SoTichKe, A.AppDesc, A.Slog, A.NgayHetHieuLuc, A.NgayTichKe, A.STO, A.NguoiDaiDien, b.TenPhuongThucXuat, 
                      c.TenNhaCungCap, A.Tax, A.PaymentMethod, d.NoiDung AS PhuongThucThanhToan, A.Term, e.GhiChu AS ThoiHanThanhToan, A.MaTuyenDuong, 
                      g.DGTuyenDuong AS TenTuyenDuong, A.MaKhoNhap, h.TenKho AS TenKhoNhap, A.SoHopDong, A.NgayHopDong, A.TyGia, A.SoTKHQNhap, A.SoTKHQXuat, 
                      A.SelfShipping, A.PriceGroup, pg.PriceGroupName, A.Inco1,
                       A.Inco2, A.DiemTraHang, a.xuathanggui, DischargePoint, DesDischargePoint
                       , a.So_TKTN, a.So_TKTX, a.Ngay_TKTX
                       ,a.XITEC_OPTION
FROM         dbo.tblLenhXuatE5 AS A WITH (Nolock) LEFT OUTER JOIN
                      dbo.tblPhuongThucXuat AS b WITH (Nolock) ON b.MaPhuongThucXuat = A.MaPhuongThucXuat 
                      and b.VTWEG=a.MaPhuongThucBan
																and isnull(b.BSART,'')=isnull(a.BSART,'')
