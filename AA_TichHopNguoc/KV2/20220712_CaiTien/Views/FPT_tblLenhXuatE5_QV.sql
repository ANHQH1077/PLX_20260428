...
alter VIEW [dbo].[FPT_tblLenhXuatE5_QV]
AS
SELECT        A.MaLenh, A.NgayXuat, A.SoLenh, A.MaDonVi, A.MaNguon,
                             ...
                             (SELECT        TOP (1) TenKhachHang
                               ...
                               WHERE        (MaKhachHang = A.MaKhachHang)) AS TenKhachHang, A.LoaiPhieu, A.Niem, A.LuongGiamDinh, A.NhietDoTaiTau, A.GhiChu, A.NgayHieuLuc, RTRIM(LTRIM(A.Status)) AS Status, A.Number, A.Createby, A.UpdatedBy,
                          A.CreateDate, A.UpdateDate, A.SoLenhSAP, CASE WHEN LoaiPhieu = 'V144' THEN 'N' ELSE 'Y' END AS ChuyenVanChuyen, A.Client, A.HTTG, A.Approved, A.User_Approve, A.Date_Approve, A.EditApprove, A.NhaCungCap,
                             (SELECT        Name
                         ...
                         d.NoiDung AS PhuongThucThanhToan, A.Term, e.GhiChu AS ThoiHanThanhToan, A.MaTuyenDuong, g.DGTuyenDuong AS TenTuyenDuong, A.MaKhoNhap, h.TenKho AS TenKhoNhap, A.SoHopDong, A.NgayHopDong, A.TyGia, 
                         A.SoTKHQNhap, A.SoTKHQXuat, A.SelfShipping, A.PriceGroup, pg.PriceGroupName, A.Inco1, A.Inco2, A.DiemTraHang, A.XuatHangGui, A.DischargePoint, A.DesDischargePoint, A.CardNum, A.CardData, A.LXPhieu, A.LXLoai, 
                         A.SOType, A.PrcingDate, A.POType, A.So_TKTX, A.Ngay_TKTX, A.PriceSynDate
						 ,(select
							   distinct  
								stuff((select ' / ' + u.TenHangHoa 
									from fpt_tblLenhXuat_HangHoaE5_v u where  solenh =a.SoLenh 
									order by u.TenHangHoa
									for xml path('')),2,2,'') ) as  TenHangHoa22
FROM            dbo.tblLenhXuatE5 AS A WITH (Nolock) LEFT OUTER JOIN
                         dbo.tblPhuongThucXuat AS b WITH (Nolock) ON b.MaPhuongThucXuat = A.MaPhuongThucXuat AND b.VTWEG = A.MaPhuongThucBan AND ISNULL(b.BSART, N'') = ISNULL(A.BSART, N'') LEFT OUTER JOIN
