...
Public Class FrmSearch
    ...
    Public Fpt_Object As U_TextBox.GridView    'Object gọi form Searchs
...
#End Region
    ...
    Private Sub p_QuerySource(ByVal p_LoadForm As Boolean)
        ...
        Try
                ...
                For p_Count = 0 To Me.GridView1.Columns.Count - 1
                    If Me.GridView1.Columns.Item(p_Count).Visible = True Then
                        If g_MultiCheck = True Then
                            ...
                            End If
                        End If
                        p_String = IIf(Me.U_TextBox1.Text.ToString.Trim = "", "*", Me.U_TextBox1.Text.ToString.Trim)

                        If UCase(g_DBTYPE) = "ORACLE" Then
                            If p_where = "" Then
                                p_where = " upper(" & Me.GridView1.Columns.Item(p_Count).FieldName & ") Like N'%" & _
                                      UCase(Replace(p_String, "*", "%", 1)) & "%'"
                            Else
                                p_where = p_where & "  OR  upper(" & Me.GridView1.Columns.Item(p_Count).FieldName & ") Like N'%" & _
                                      UCase(Replace(p_String, "*", "%", 1)) & "%'"
                            End If
                        Else
                            If p_where = "" Then
            ...
            '        If p_ColumnWidth(p_Count) > 0 And p_ColumnWidth.Length >= p_Count Then
            '            Me.GridView1.Columns.Item(p_Count).Width = p_ColumnWidth(p_Count)
            '        End If

            '    End If
            'Next
            For p_Count = 0 To p_ColumnWidth.Length - 1
                If p_ColumnWidth(p_Count).ToString.Trim <> "" Then
    ...
    Public Function p_Set_TrueGrid_Property_LoadPage(ByRef p_DesError As String, _
        ...
        Dim p_Source As New BindingSource

        Dim p_Table As DataTable

        Try

            p_GridView.OptionsBehavior.AllowAddRows = DevExpress.Utils.DefaultBoolean.True
            p_GridView.OptionsBehavior.AllowDeleteRows = DevExpress.Utils.DefaultBoolean.False
            ...
            If UCase(g_DBTYPE) = "ORACLE" Then
                If p_FieldKey <> "" And p_LoadPage = True Then
                    ...
                    p_SQL = "SELECT anhqh.* FROM (" & p_SQLString & ") anhqh ORDER BY " & p_FieldKey & ""

                    p_SQL = "SELECT * FROM (" & p_SQL & ") anhqh1 WHERE ROWNUM BETWEEN (" & p_PageNum & " -1)* " & p_LineOfPage & " +1  " & _
                ...
                p_SQLTotal = "SELECT ceil( COUNT(*)/" & p_LineOfPage & ")  as TotalPage  FROM (  " & p_SQLString & " ) A"

                p_Table = g_Service.clsGetDataTableOracle(p_SQL, p_SQL)
                p_Source.DataSource = p_Table

                
                p_DataTmp = g_Service.clsGetDataTableOracle(p_SQLTotal, p_SQL)
                ...
                '  p_DataTmp = g_Service.clsGetDataSet_Oracle(p_SQL & ";|| chr(13) || chr(10) ||" & p_SQLTotal & ";", p_SQL)
            Else
                ...
                p_Table = g_Service.Sys_SYS_GET_DATATABLE_Des(p_SQL, p_SQL)
                p_Source.DataSource = p_Table
            ...
            '    End If
                ...
                p_TrueGird.DataSource = p_Source   ' p_BindingSource
    ...
    Sub p_ChooseDoubleClick()
        ...
        If GridView1.FocusedRowHandle >= 0 Then
            If InStr(UCase(Fpt_Object.ToString), UCase(".GridView"), CompareMethod.Text) > 0 Then   'Return cho Grid
                    ...
                    If Not p_ObjSearchReturn(p_Count) Is Nothing Then
                        If p_ObjSearchReturn(p_Count).Trim <> "" Then
                            Try
                                ...
                                Fpt_Object.SetFocusedRowCellValue(p_ObjSearchReturn(p_Count).Trim, p_DataRow.Item(p_Count))

                                'p_ChooseRecord.editvalue = "Y"
                                g_ChooseRecordFromSearch = True
                            Catch ex As Exception
