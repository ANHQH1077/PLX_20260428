Module ChungTuHoaDon
        ...
        Try
            ...
            Else
                Try
                    ...
                    If DtTable.Rows(0).Item("ChuyenVanChuyen").ToString.Trim = "Y" Then
                        p_RptHoasDon.Parameters.Item("P_LenhXuat").Value = ""
                    Else
    ...
    Public Sub In_HoaDon_HangGiuHo(ByVal p_Form As Object, ByVal W_SoLenh As String, _
        ...
        p_SQL = "select TenDonVi from tblDonvi where MaDonVi  in (select companycode from tblconfig)"
        p_Table = GetDataTable(p_SQL, p_SQL)

        If Not p_Table Is Nothing Then
            If p_Table.Rows.Count > 0 Then
                p_TenDonVi = p_Table.Rows(0).Item("TenDonVi").ToString.Trim
            End If
        End If
        ...
        ' rpt.Parameters.Item("CongTy").Value = p_TenDonVi   '"CÔNG TY XĂNG DẦU TÂY NAM BỘ" 'ToUpper()

        srt_dt = Format(p_dtNgayXuat).ToString
        srt_dieudong = Format(p_dtpDieuDong).ToString
        rpt.Parameters.Item("P_Ngay").Value = p_dtNgayXuat.ToString("dd")
        rpt.Parameters.Item("P_Thang").Value = p_dtNgayXuat.ToString("MM")
        rpt.Parameters.Item("P_Nam").Value = p_dtNgayXuat.ToString("yyyy")

        rpt.Parameters.Item("P_NgayDieuDong").Value = p_dtpDieuDong.ToString("dd")
        rpt.Parameters.Item("P_ThangDieuDong").Value = p_dtpDieuDong.ToString("MM")
        rpt.Parameters.Item("P_NamDieuDong").Value = p_dtpDieuDong.ToString("yyyy")
