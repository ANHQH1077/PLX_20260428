...
Module HTTG_SCADAR
    ...
    Public g_ConfigDecima As DataTable = Nothing
    ...
    Public p_INTANK_E5_TDH As Boolean = False
    Public p_INTANK_E5 As Boolean = False

    '1

    'Ham dung de cap nhat STT xep xe cho don vi Phu Duc
    Private Sub PhuDucCapNhatSo_TT(ByRef p_DataTable As DataTable, ByRef p_DataTable_SOTT As DataTable)
        Dim p_Count As Integer
        Dim p_FoxDataTable As DataTable
        Dim p_SQL As String
        Dim p_Desc As String = ""
        Dim p_SoTT As Integer = 0
        Dim p_Count_STT As Integer
        Dim p_Row As DataRow

        If p_DataTable.Columns.IndexOf("WareHouse") > 0 Then
            If p_DataTable.Rows(0).Item("WareHouse").ToString.Trim = "2011" Then
                For p_Count = 0 To p_DataTable.Rows.Count - 1
                    If p_DataTable.Rows(p_Count).Item("SO_TT").ToString.Trim <> "" Then
                        If p_DataTable.Rows(p_Count).Item("SO_TT") > 0 Then
                            Continue For
                        End If
                    End If
                    p_SQL = "select Max(SO_TT)  as SO_TT from  " & g_PathFileFoxOut & " Where Hong_Xuat ='" & p_DataTable.Rows(p_Count).Item("HongXuat").ToString.Trim & "'"
                    p_FoxDataTable = p_SYS_GET_DATATABLE_With_Connect_Des(g_StrConnectFox, p_SQL, p_Desc)
                    p_SoTT = 0
                    If Not p_FoxDataTable Is Nothing Then
                        If p_FoxDataTable.Rows.Count > 0 Then
                            Integer.TryParse(p_FoxDataTable.Rows(0).Item(0).ToString.Trim, p_SoTT)
                        End If
                    End If

                    p_SoTT = p_SoTT + 1
                    p_DataTable.Rows(p_Count).Item("SO_TT") = p_SoTT
                    p_SQL = "Update tblLenhXuatChiTietE5 set SO_TT=" & p_SoTT & _
                            ",MaLuuLuongKe='" & p_DataTable.Rows(p_Count).Item("HongXuat").ToString.Trim & "' where Row_ID=" & p_DataTable.Rows(p_Count).Item("Row_ID").ToString.Trim
                    p_Row = p_DataTable_SOTT.NewRow
                    p_Row(0) = p_SQL
                    p_DataTable_SOTT.Rows.Add(p_Row)

                    For p_Count_STT = p_Count + 1 To p_DataTable.Rows.Count - 1
                        If p_DataTable.Rows(p_Count).Item("HongXuat").ToString.Trim = p_DataTable.Rows(p_Count_STT).Item("HongXuat").ToString.Trim Then
                            p_SoTT = p_SoTT + 1
                            p_DataTable.Rows(p_Count_STT).Item("SO_TT") = p_SoTT
                            p_SQL = "Update tblLenhXuatChiTietE5 set SO_TT=" & p_SoTT & _
                                    ",MaLuuLuongKe='" & p_DataTable.Rows(p_Count).Item("HongXuat").ToString.Trim & "' where Row_ID=" & p_DataTable.Rows(p_Count_STT).Item("Row_ID").ToString.Trim
                            p_Row = p_DataTable_SOTT.NewRow
                            p_Row(0) = p_SQL
                            p_DataTable_SOTT.Rows.Add(p_Row)
                        End If
    ...
    'Ham dung de cap nhat STT xep xe cho don vi Phu Duc
    Private Sub PhuDucCapNhatSo_TT_Backup(ByRef p_DataTable As DataTable, ByRef p_DataTable_SOTT As DataTable)
        Dim p_Count As Integer
        Dim p_FoxDataTable As DataTable
        Dim p_SQL As String
        Dim p_Desc As String = ""
    ...
    End Sub
    Public Function mdlHTTG_To_Scadar(ByVal p_TypeIn As String, ByVal p_SoLenh As String, ByVal g_LoaiVanChuyen As String, _
        ...
        Try
            ...
            If Not p_DataSet Is Nothing Then
                If p_DataSet.Tables.Count > 0 Then
                    ...
                    'p_INTANK_E5_TDH
                    p_INTANK_E5_TDH = False
                    p_ArrRow = g_Table_Sys_Config.Select("KEYCODE='INTANK_E5_TDH'")
                    If p_ArrRow.Length > 0 Then
                        If p_ArrRow(0).Item("KEYVALUE").ToString.Trim = "Y" Then
                            p_INTANK_E5_TDH = True
                        End If
                    End If

                    g_Convert_Font = False
                    p_ArrRow = g_Table_Sys_Config.Select("KEYCODE='CONVERT_FONT'")
                    If p_ArrRow.Length > 0 Then
                        If p_ArrRow(0).Item("KEYVALUE").ToString.Trim = "Y" Then
                            g_Convert_Font = True
                        End If
    ...
    Private Sub SQLGetDataToScadar(ByVal p_TypeIn As String, ByVal p_SoLenh As String, ByVal g_LoaiVanChuyen As String, ByRef p_Error As Boolean, ByRef p_Desc As String, _
        ...
        Dim p_DataTable_SOTT As DataTable
        ...
        p_DataTable_SOTT = p_TableExec.Clone
        ...
        If Not p_DataHTTG Is Nothing Then
            ...
            'Dung cho phu duc neu co

            PhuDucCapNhatSo_TT(p_DataHTTG, p_DataTable_SOTT)
            ...
            For p_Count = 0 To p_DataHTTG.Rows.Count - 1
                ...
                For p_CountRow = 0 To p_DataRowMap_cp.Length - 1
                    ...
                    If p_DataRowMap_cp(p_CountRow).Item("Bo").ToString.Trim = "FlagTankLine" Then
                        p_FieldType = p_FieldType
                    End If
                    ''
                    ...
                    If p_FieldType.ToString.Trim <> "" Or UCase(p_DataRowMap_cp(p_CountRow).Item("FromField").trim) = "FLAG1" Then
                        ...
                        Select Case UCase(p_FieldType)
                            ...
                            Case UCase("String")
                                ...
                                If g_Convert_Font = False Then
                                    p_SQLValue = p_SQLValue & ",'" & p_Value & "'"
                                Else

                                    p_SQLValue = p_SQLValue & ",'" & StringConvertFontNew(p_Value) & "'"
                                End If
            ...
            If g_KV1 = True Then
                ...
                " NgayTichKe=getdate() , CreateDate =getdate() where charindex(',' + SoLenh + ',','" & p_SoLenh & ",',1)>0 "
            Else
                p_DataRow.Item(0) = "UPDATE tblLenhXuatE5 set Status='3',UserTichKe='" & g_UserName & "', " & _
            ...
            If p_DataTable_SOTT.Rows.Count > 0 Then
                For p_CountRow = 0 To p_DataTable_SOTT.Rows.Count - 1
                    p_DataRow = p_TableExec.NewRow
                    p_DataRow(0) = p_DataTable_SOTT.Rows(p_CountRow).Item(0).ToString.Trim
                    p_TableExec.Rows.Add(p_DataRow)
    ...
    'anhqh
    'Ham thuc hien lay du lieu chi tiet cua HTTG de day sang Scadar
    'Dung cho truong hop xuat Intank
    Private Sub SQLGetDataToScadar_InTank(ByVal p_TypeIn As String, ByVal p_SoLenh As String, ByVal g_LoaiVanChuyen As String, ByRef p_Error As Boolean, ByRef p_Desc As String, _
                                    ByVal p_Terminal As String)
        Dim p_SQL As String
        Dim p_DataHTTG As DataTable
        Dim p_CountRow As Integer
        ...
        If Not p_DataHTTG Is Nothing Then
            ...
            For p_Count = p_DataHTTG.Rows.Count - 1 To 0 Step -1
                If CheckHangHoaE5(p_DataHTTG.Rows(p_Count).Item("MaHangHoa").ToString.Trim) = False Then
                    ' p_DataHTTG.Rows(p_Count).Item("MaHangHoa") = "0201001"
                    p_DataHTTG.Rows.RemoveAt(p_Count)

                Else
                    If Len(p_DataHTTG.Rows(p_Count).Item("MaHangHoa")) > 6 Then
                        p_DataHTTG.Rows(p_Count).Item("MaHangHoa") = Left(p_DataHTTG.Rows(p_Count).Item("MaHangHoa"), 5) & Right(p_DataHTTG.Rows(p_Count).Item("MaHangHoa"), 1)
                    End If
                End If
            Next

            p_DataHTTG.AcceptChanges()
                        ...
                        Select Case UCase(p_FieldType)
                            ...
                            Case UCase("String")
                                ...
                                Else
                                    ...
                                    p_SQLValue = p_SQLValue & ",'" & StringConvertFont(p_Value) & "'"
                                End If
    ...
    Public Function GetConnectFox(ByRef p_DessErr As String, ByVal p_TypeIn As String, ByVal g_LoaiVanChuyen As String, Optional ByVal p_Terminal As String = "") As Boolean
        ...
        If g_Table_Sys_Config Is Nothing Then
            ...
            g_Table_Sys_Config = g_Services.Sys_SYS_GET_DATATable_Des(p_SQL, p_SQL)
        ...
        Else
            ...
            g_DataMap_cp = g_Services.Sys_SYS_GET_DATATable_Des(p_SQL, p_SQL)
        End If
