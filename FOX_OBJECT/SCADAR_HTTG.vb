Module SCADAR_HTTG
    ...
    Private p_DataRowArr() As DataRow
    ...
    Sub CapnhatTyTrongInTank(p_SoLenh As String)
        Dim p_Table As DataTable
        Dim p_SQL As String
        Dim p_Count As Integer
        Dim p_TableExe As New DataTable
        Dim p_Row As DataRow
        p_TableExe.Columns.Add("SCRIPT_SQL")


        p_SQL = "select Row_ID , BeXuat from FPT_tblLenhXuatChiTietE5_V  where SoLenh='" & p_SoLenh & "' and FlagTankLine = 0 and isnull( TyLe_TTe ,0) <=1  "
        p_Table = GetDataTable(p_SQL, p_SQL)
        If Not p_Table Is Nothing Then
            For p_Count = 0 To p_Table.Rows.Count - 1
                p_SQL = "Update tblLenhXuatChiTietE5 set  GST = 0,  TyTrong_15=isnull((select top 1 Dens_nd from FPT_tblTankActive_V bb " & _
                                  "where Date1 =CONVERT(date,getdate()) and Name_nd = '" & p_Table.Rows(p_Count).Item("BeXuat") & "' ),0)  " & _
                      " where Row_ID = " & p_Table.Rows(p_Count).Item("Row_ID")
    ...
    Private Sub SQLGetDataFromScadar(ByVal p_TypeIn As String, ByVal p_SoLenh As String, ByVal g_LoaiVanChuyen As String, ByRef p_Error As Boolean, ByRef p_Desc As String, _
        ...
        Dim p_TableID_Scadar As String = ""
                        ...
                        If UCase(p_DataRowMap_cp(p_CountRow).Item("FromField").trim) = UCase("NgayXuat") And p_DataRowMap_cp(p_CountRow).Item("SWhere").ToString.Trim = "Y" Then
                            If p_Where_Check.ToString.Trim = "" Then
                                ...
                                If UCase(g_TypeConnet) = "FOX" And p_HangHoaE5 = False Then
                                    ...
                                    If UCase(p_FieldType.ToString.Trim) = "STRING" Then
                                        'CTOD
                                        Select Case UCase(g_LoaiVanChuyen)
                                            Case "BO"
                                                p_Where_Check = "CTOD (" & p_DataRowMap_cp(p_CountRow).Item("Bo").trim & ") ={d'" & CDate(p_Value.ToString.Trim).ToString("yyyy-MM-dd") & "'}"
                                            Case "THUY"
                                                p_Where_Check = "CTOD (" & p_DataRowMap_cp(p_CountRow).Item("Bo").trim & ") ={d'" & CDate(p_Value.ToString.Trim).ToString("yyyy-MM-dd") & "'}"
                                            Case "SAT"
                                                p_Where_Check = "CTOD (" & p_DataRowMap_cp(p_CountRow).Item("Bo").trim & ") ={d'" & CDate(p_Value.ToString.Trim).ToString("yyyy-MM-dd") & "'}"
                            ...
                            Else
                                ...
                                If UCase(g_TypeConnet) = "FOX" And p_HangHoaE5 = False Then
                                    ...
                                    If UCase(p_FieldType.ToString.Trim) = "STRING" Then
                                        Select Case UCase(g_LoaiVanChuyen)
                                            Case "BO"
                                                p_Where_Check = p_Where_Check & " AND CTOD (" & p_DataRowMap_cp(p_CountRow).Item("Bo").trim & ")={d'" & CDate(p_Value.ToString.Trim).ToString("yyyy-MM-dd") & "'}"
                                            Case "THUY"
                                                p_Where_Check = p_Where_Check & "  AND  CTOD (" & p_DataRowMap_cp(p_CountRow).Item("Thuy").trim & ")={d'" & CDate(p_Value.ToString.Trim).ToString("yyyy-MM-dd") & "'}"
                                            Case "SAT"
                                                p_Where_Check = p_Where_Check & "  AND  CTOD (" & p_DataRowMap_cp(p_CountRow).Item("Sat").trim & ")={d'" & CDate(p_Value.ToString.Trim).ToString("yyyy-MM-dd") & "'}"
                                            Case Else
                                                Continue For
                                        End Select
                                    Else
...
HoangHoaPhaChe:
                        ...
                        If UCase(p_DataRowMap_cp(p_CountRow).Item("FromField").trim) = "MATUDONGHOA" And p_DataRowMap_cp(p_CountRow).Item("SWhere").ToString.Trim = "Y" Then
                            If p_HangHoaE5 = False Or p_MaHangHoa = "0201001" Or p_MaHangHoa = "020202" Then
                ...
                If p_Where_Check.ToString.Trim <> "" Then
                    ...
                    Else
                        ...
                        'anhqh
                        '20161130
                        'Ham kiem tra xang thuong xuat hong E5 truoc, neu khong co thi kiem tra trong hong xang thuong
                        ...
                        If UCase(g_TypeConnet) = "FOX" Then
                            ...
                            If Not p_DataTableA92 Is Nothing Then
                                If p_DataTableA92.Rows.Count <= 0 Then
                                    p_DataTableCheckID = p_SYS_GET_DATATABLE_With_Connect_Des(g_StrConnectFox, p_SQL, p_SQL)
                                    If Not p_DataTableCheckID Is Nothing Then
                                        If p_DataTableCheckID.Rows.Count > 0 Then
                                            If p_TableScadar Is Nothing Then
    ...
    Private Function SQLGetDataFromScadar_A92(ByVal p_DataHTTG As DataTable, ByVal p_TypeIn As String, ByVal p_SoLenh As String, ByVal g_LoaiVanChuyen As String, ByRef p_Error As Boolean, ByRef p_Desc As String, _
        ...
        '    End If
        'Else
        If g_ConnectToScadar_E5.ToString.Trim = "" Then
            p_Error = True
            p_Desc = "String kết nối đến máy chủ Scadar không xác định"
            Exit Function
        End If
                ...
                For p_CountRow = 0 To p_DataRowMap_cp.Length - 1
                    ...
                    End If
                        ...
                        If UCase(p_DataRowMap_cp(p_CountRow).Item("FromField").trim) = "MAHANGHOA" And UCase(g_LoaiVanChuyen) = "SAT" Then
                            ...
                            End If
                        End If
                        If UCase(p_DataRowMap_cp(p_CountRow).Item("FromField").trim) = "TABLEID" And p_DataRowMap_cp(p_CountRow).Item("SWhere").ToString.Trim = "Y" And UCase(g_TypeConnet) = "SQL" Then
                            ' If g_MaTuDongHoa = "N" Then  'Ma tu dong hoa cho SQL la TableID
                            p_Value = p_DataRowHTTG.Item("TableID").ToString.Trim
                            'If g_MaTuDongHoa = "N" Then
                            If p_Value.ToString.Trim <> "" Then
                        ...
                        If UCase(p_DataRowMap_cp(p_CountRow).Item("FromField").trim) = "MATUDONGHOA" And p_DataRowMap_cp(p_CountRow).Item("SWhere").ToString.Trim = "Y" Then
                            'anhqh
                            '20161130
                            'Kiem tra neu khong co so luong thuc xuat thi out

                            'If p_DataRowHTTG.Item("GV").ToString.Trim = "" Or p_DataRowHTTG.Item("GV").ToString.Trim = "0" Or p_DataRowHTTG.Item("GV").ToString.Trim = "0.0" Then
                            '    GoTo line_tt
                            'Else
                            p_Value = p_DataRowHTTG.Item(p_DataRowMap_cp(p_CountRow).Item("FromField").trim).ToString.Trim
                            If p_Where_Check.ToString.Trim = "" Then
                            ...
                            If p_Where_Check.ToString.Trim = "" Then
                                        ...
                                        p_Where_Check = " isnull(GV,0) > 0 "
                                   
                            Else
                                p_Where_Check = p_Where_Check & " and isnull(GV,0) > 0 "
                            End If
    ...
    Private Function UpdateFromScadarToHTTG(ByVal g_LoaiVanChuyen As String, ByVal p_DataRowMap_cp_E5() As DataRow, ByVal p_DataRowMap_cp_Old() As DataRow, ByVal p_DataTableHTTG As DataTable, _
        ...
        Try
            ...
            For p_CountHTTG = 0 To p_DataTableHTTG.Rows.Count - 1
                ...
                If CheckHangHoaE5(p_MaHangHoa) And g_HTTG_E5 = True Then
                    'End If
                    p_STT = p_DataRowMap_cp_E5(0).Item("STT").ToString.Trim
                    p_DataRowMap_cp = g_DataMap_Line_cp.Select("STT=" & p_STT)
                    p_HangHoaE5 = True
                        ...
                        Else
                            ...
                            Else
                                ...
                                If p_ColumnType = "" Then
                                    'anhqh
                                    '20170601
                                    'voi truong hop Lai cai thif file Scadar khong co thong tin ngay xuat trong fox
                    ...
                    If UCase(p_DataRowMap_cp(p_CountRow).Item("FromField").trim) = "MATUDONGHOA" And p_DataRowMap_cp(p_CountRow).Item("SWhere").ToString.Trim = "Y" Then
                        If p_HangHoaE5 = False Or p_MaHangHoa = "0201001" Or p_MaHangHoa = "020202" Then
                            ...
                            If p_Scadar Is Nothing Then
                                ''anhqh
                                '20170313
                                'Doan nay bi dinh voi KV3 khi xang A92 va Xang E5 deu xuat hong E5
                                GoTo Line_TT
                                'UpdateFromScadarToHTTG = True
                                'Exit Function
                            End If
                ...
                For p_CountRow = 0 To p_DataRowMap_cp.Length - 1
                    ...
                    If UCase(p_DataRowMap_cp(p_CountRow).Item("FromField").trim) = "MATUDONGHOA" And p_DataRowMap_cp(p_CountRow).Item("SWhere").ToString.Trim = "Y" Then
                        ...
                        If p_Where_Scadar.ToString.Trim = "" Then
                            Select Case UCase(g_LoaiVanChuyen)
                                Case "BO"
                                    p_Where_Scadar = p_DataRowMap_cp(p_CountRow).Item("Bo").trim & "=" & p_Value & ""
                                Case "THUY"
                                    p_Where_Scadar = p_DataRowMap_cp(p_CountRow).Item("Thuy").trim & "=" & p_Value & ""
                                Case "SAT"
                                    p_Where_Scadar = p_DataRowMap_cp(p_CountRow).Item("Sat").trim & "=" & p_Value & ""
                            End Select
                        Else
                            Select Case UCase(g_LoaiVanChuyen)
                                Case "BO"
                                    p_Where_Scadar = p_Where_Scadar & " AND " & p_DataRowMap_cp(p_CountRow).Item("Bo").trim & "=" & p_Value & ""
                                Case "THUY"
                                    p_Where_Scadar = p_Where_Scadar & "  AND " & p_DataRowMap_cp(p_CountRow).Item("Thuy").trim & "=" & p_Value & ""
                                Case "SAT"
                                    p_Where_Scadar = p_Where_Scadar & "  AND " & p_DataRowMap_cp(p_CountRow).Item("Sat").trim & "=" & p_Value & ""
                            End Select
                        End If
                    ...
                    Select Case UCase(p_ColumnType)
                        ...
                        Case UCase("DateTime"), UCase("Date")
                            ...
                            Else
                                'anhqh
                                '20161104
                                'Union gia tri ngay va gio cua scadar Fox
                                p_DateTmp = UnionDateTime_Fox(p_DateTime, p_Value)

                                p_SQL = p_SQL & "," & p_FieldName & "='" & CDate(p_DateTmp).ToString("yyyyMMdd hh:mm:ss tt") & "'"
                                'p_SQL = p_SQL & "," & p_FieldName & "='" & CDate(p_Value).ToString("yyyyMMdd hh:mm:ss tt") & "'"
                            End If
...
HoangHoaPhaChe:
    ...
    Public Function UnionDateTime_Fox(ByVal p_Date As Date, ByVal p_Time As String) As DateTime
        Dim p_Value As String

        UnionDateTime_Fox = p_Date
        Try
            p_Value = ""
            p_Value = p_Date.ToString("MM/dd/yyyy") & " " & p_Time
            UnionDateTime_Fox = CDate(p_Value)
        Catch ex As Exception
