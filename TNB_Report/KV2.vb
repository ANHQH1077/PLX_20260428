...
Module KV2
    ...
    Public Function mdlTichke_PrintTrans_new(ByVal i_Preview As Boolean, ByVal i_dt_trans As DataTable, _
        ...
        Try
            ...
            Dim exportMethodName As String

            'If i_maphuongthucban.Contains("03") Then
            '    exportMethodName = "Di chuyển/Bán NBộCty"
            'Else
            '    exportMethodName = "Bán cho khách hàng"
            'End If


            Dim symbol As String = CultureInfo.CurrentCulture.NumberFormat.NumberDecimalSeparator
            Dim digitSymbol As String = CultureInfo.CurrentCulture.NumberFormat.NumberGroupSeparator

            Dim l_header() As String

            If symbol.Contains(",") Then
                l_header = New String() {l_ngayxuat, _
            ...
            Else
                l_header = New String() {l_ngayxuat, _
                                                    ...
                                                    i_dt_trans.Rows(0).Item("MaPhuongTien").ToString(), _
                                                    i_dt_trans.Rows(0).Item("NguoiVanChuyen").ToString(), _
                                                    mdlFunction_Convert2DecimalString(l_TongXuat), _
                                                    exportMethodName, _
                                                    ""}
            End If
            '-------------------------------------------------------------------------
        ...
        Try
            If i_Preview Then
                If i_dt_trans.Rows(0).Item("Bo").ToString().Trim() = "X" Then
                    _rptTicke_new = New rptTicke_new()
                    ...
                    _rptTicke_new.SetParameterValue("P_PhuongThucBan", i_header(6))
                    _rptTicke_new.SetParameterValue("P_ChuyenVanTai", i_header(7))
                    _rptTicke_new.PrintOptions.PrinterName = g_DefaultPrint
                ...
                Else
                    ...
                    _rptTicke_thuy.SetParameterValue("P_PhuongThucBan", i_header(6))
                    _rptTicke_thuy.SetParameterValue("P_ChuyenVanTai", i_header(7))
                    _rptTicke_thuy.PrintToPrinter(1, False, 0, 0)

                End If
            Else
                Dim l_fprint As frmPrint = New frmPrint(1, i_dt_trans, i_header)
                l_fprint.ShowDialog()
            End If
