Public Class FrmLenhXuat_NoiDiaDO1
    ...
    Private Sub FrmLenhXuat_NoiDiaDO1_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        'p_SQL = "select   Vbeln as SoHopDong, Kunnr as MaKhachHang, Inco1 from dbo.tblProjects with (nolock) where  Vkorg not in ('" & g_Company_Code & "')  and " & _
        ...
        p_SQL = "select   Vbeln as SoHopDong,  ZLSCH as PaymentMethod,ZTERM as Term, INCO2 as Inco2, Inco1 from dbo.tblProjects with (nolock) where  Vkorg  in ('" & g_KhoNhap & "')  and " & _
                   "convert(date,getdate()) >= convert(date, case when isdate(Guebg) =1 then convert(date,Guebg)  else getdate() end ) " & _
              "and convert(date,getdate()) <= convert(date, case when isdate(Gueen) =1 then convert(date,Gueen)  else getdate() end) " & _
        ...
        If p_SoLenh = "" Then
            ...
            If p_DataTableLX.Rows.Count > 0 Then
                ...
                Me.MaKhoXuat.EditValue = p_DataTableLX.Rows(0).Item("MaKhoNhap").ToString
                Me.TenKhoXuat.EditValue = p_DataTableLX.Rows(0).Item("TenKhoNhap").ToString
        ...
        If Me.FormStatus = True Then
            ...
            If Me.FormStatus = False Then
                ...
                If p_DO1 <> "" And p_DO2 <> "" Then
                    ...
                    If Not p_DataTable Is Nothing Then
                        If p_DataTable.Rows.Count > 0 Then
                            p_SQL = "update tblLenhXuate5  set DO1_SoLenh ='" & p_DO1 & "', PriceGroupDO1 = '" & p_PriceGroup & "'  where  SoLenh  ='" & p_DO2 & "'  "
    ...
    Private Sub MaKhachHang_EditValueChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles MaKhachHang.EditValueChanged
        Dim p_Date As U_TextBox.U_ButtonEdit
        p_Date = CType(sender, U_TextBox.U_ButtonEdit)
        If p_Date.IsModified = True Then
            ResetPrice()
        End If
    End Sub
    ...
    Private Sub ResetPrice()
        ...
        If Not p_DataTable Is Nothing Then
            If p_DataTable.Rows.Count > 0 Then
                p1_Company_Code = p_DataTable.Rows(0).Item("MaDonVi").ToString.Trim
            End If
        End If
        ...
        If Not MaPhuongThucBan.EditValue Is Nothing Then
            p_PhuongThucBan = MaPhuongThucBan.EditValue.ToString.Trim()
        End If
    ...
    Private Sub U_ButtonEdit3_EditValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles U_ButtonEdit3.EditValueChanged
        Dim p_MaKhoXuat As U_TextBox.U_ButtonEdit
        p_MaKhoXuat = CType(sender, U_TextBox.U_ButtonEdit)
        If p_MaKhoXuat.IsModified = True Then
            ResetPrice()
    ...
    Private Sub SoHopDong_EditValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles SoHopDong.EditValueChanged
        ...
        If p_ButtonEdit.IsModified Then
            ...
            End If
            p_SQL = " select  convert(date,case when isdate(Guebg) =0 then null else  Guebg end) as NgayHopDong  " & _
                            " from dbo.tblProjects with (nolock)  where Vbeln ='" & p_SoHD & "' and isdate(Guebg) =1  "
    ...
    Private Sub Term_Validating(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles Term.Validating
        Dim p_MaKhoXuat As U_TextBox.U_ButtonEdit
        p_MaKhoXuat = CType(sender, U_TextBox.U_ButtonEdit)
        If p_MaKhoXuat.IsModified = True Then
            ResetPrice()
    ...
    Private Sub SoHopDong_LostFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles SoHopDong.LostFocus
