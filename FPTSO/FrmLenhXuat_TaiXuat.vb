Public Class FrmLenhXuat_TaiXuat
    Public g_LXLoai As String = "CIF"
    Public g_LXPhieu As String = "TAIXUAT"
    ...
    Private Sub SoTonToKhai()
        Dim p_SQL As String
        Dim p_Table As DataTable
        Dim p_TK_NUMBER As String = ""
        Dim p_date As DateTime
        Dim p_R_KWMENG2 As Double = 0
        Dim p_SoLenh As String = ""
        If Not Me.SoLenh.EditValue Is Nothing Then
            p_SoLenh = Me.SoLenh.EditValue.ToString.Trim
        End If
        If Not Me.SoTKHQNhap.EditValue Is Nothing Then
            p_TK_NUMBER = Me.SoTKHQNhap.EditValue.ToString.Trim
        End If
        Me.TK_SynDate.EditValue = Nothing
        If p_TK_NUMBER <> "" Then
            p_SQL = " Exec   FPT_SoTonToKhaiTaiXuat '" & p_TK_NUMBER & "','" & p_SoLenh & "' "
            p_Table = GetDataTable(p_SQL, p_SQL)
            If Not p_Table Is Nothing Then
                If p_Table.Rows.Count > 0 Then
                    Double.TryParse(p_Table.Rows(0).Item(1).ToString.Trim, p_R_KWMENG2)
                    Me.TK_SynDate.EditValue = CDate(p_Table.Rows(0).Item(0))
                End If
            End If
        End If
       
        Me.R_KWMENG2.EditValue = p_R_KWMENG2
    End Sub

    Private Function KiemTraTonKhoToKhai() As Boolean
        Dim p_SQL As String = ""
        Dim p_Table As DataTable
        Dim p_TK_NUMBER As String = ""
        Dim p_R_KWMENG2 As Double = 0
        ...
        Try
            ...
            If Not Me.R_KWMENG2.EditValue Is Nothing Then
                Double.TryParse(Me.R_KWMENG2.EditValue.ToString.Trim, p_R_KWMENG2)
            End If
            ...
            If p_TK_NUMBER <> "" Then
                ...
                KiemTraTonKhoToKhai = False
                p_SQL = " select MaHangHoa  from tblToKhaiTaiXuat  k1  with (nolock)  where  k1.TK_NUMBER ='" & p_TK_NUMBER & "'"
                p_Table = GetDataTable(p_SQL, p_SQL)
                If Not p_Table Is Nothing Then
                    If p_Table.Rows.Count > 0 Then
                        p_MahangHoa = p_Table.Rows(0).Item("MaHangHoa").ToString.Trim
                        If p_MahangHoa <> "" Then
                            For p_Count = 0 To Me.GridView1.RowCount - 1
                                p_Row = Me.GridView1.GetDataRow(p_Count)
                                If Not p_Row Is Nothing Then
                                    If p_MahangHoa = p_Row.Item("MaHangHoa").ToString.Trim Then
                                        ''If p_Row.Item("DonViTinh").ToString.Trim = "L" Then
                                        'If Strings.Left(p_MahangHoa, 2) = "07" Then
                                        '    Double.TryParse(p_Row.Item("QCI_KG").ToString.Trim, p_Value)
                                        '    p_ToTal = p_ToTal + p_Value
                                        'Else
                                        '    Double.TryParse(p_Row.Item("QCI_L15").ToString.Trim, p_Value)
                                        '    p_ToTal = p_ToTal + p_Value
                                        'End If
                                        ''Else
                                        ''   Double.TryParse(p_Row.Item("TongDuXuat").ToString.Trim, p_Value)
                                        ''p_ToTal = p_ToTal + p_Value
                                        '' End If
                                        Double.TryParse(p_Row.Item("QCI_KG").ToString.Trim, p_Value)
                                        p_ToTal = p_ToTal + p_Value
                                ...
                                End If
                            Next
                            If p_ToTal > p_R_KWMENG2 Then
                                ShowMessageBox("", "Lượng xuất lớn hơn lượng tồn của tờ khai")
                                Exit Function
                            Else
                                KiemTraTonKhoToKhai = True
                            End If
                        End If
                    End If
                End If
    ...
    Private Sub FrmTaoMoiLenhXuat_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        'p_SQL = "select TK_NUMBER, isnull( R_KWMENG2,0) -isnull(SLQC,0) as R_KWMENG2, SYNDATE  from tblToKhaiTaiXuat  k1  with (nolock) " & _
        '                 "left join  ( select MaHangHoa, DonViTinh, case when DonViTinh  ='L' then SLQC  else TongDuXuat  end as SLQC " & _
        '                   "	from (select MaHangHoa, DonViTinh , sum(isnull(TongDuXuat,0)) as TongDuXuat,  " & _
        '                       "	sum(isnull(QCI_L15,0)) as QCIL15,  " & _
        '                       "	sum(isnull(QCI_KG,0)) as QCIKG " & _
        '                       "	, case when left(MaHangHoa,2) ='07' then  sum(isnull(QCI_KG,0)) " & _
        '                        "	else  sum(isnull(QCI_L15,0))   end as SLQC " & _
        '                         "	from  tblLenhXuat_HangHoaE5 k2 with (nolock) " & _
        '                       "	where exists (select 1 from  tbllenhxuate5 kk where k2.solenh =kk.solenh " & _
        '                          "	and isnull(kk.SoTKHQNhap,'') <> '' 		and kk.Status  <> '5' " & _
        '                          "	and kk.MaPhuongThucBan in ('05','06') " & _
        '                          " and  exists (select 1 from  tblToKhaiTaiXuat  k2  with (nolock) " & _
        '                                     " where k2.TK_NUMBER = isnull(kk.SoTKHQNhap,'') " & _
        '                                     " and k2.SYNDATE  <= kk.CreateDate) ) " & _
        '                                                   "	group by MaHangHoa, DonViTinh " & _
        '                       "	) abc )  k2  on k1.MahangHoa=k2.MahangHoa  and isnull(R_KWMENG2,0) >= isnull(k2.SLQC,0) "

        p_SQL = "select TK_NUMBER as ToKhai,  SYNDATE as NgayThang, convert(varchar(8),getdate() ,114) as Gio, MaHangHoa from tblToKhaiTaiXuat  k1 " & _
                           "with (nolock)  where    convert(date,getdate() ) >=convert(date, User_F_Date) " & _
                            "and  convert(date,getdate() ) <=convert(date, User_T_Date) " & _
                            "and isnull(R_KWMENG2,0) >0	and exists (select 1 from tblconfig  k  where k1.BUKRS =k.CompanyCode)"
        Me.SoTKHQNhap.SqlString = p_SQL

            ToolStripButton4.Visible = False

            If g_LXLoai = "CIF" Then
            ...
            'p_String = "select  Vbeln as SoHopDong, Kunnr as MaKhachHang, Inco1 from dbo.tblProjects with (nolock) where convert(date,getdate()) >= " & _
            '        "convert(date,Guebg) and convert(date,getdate()) <= convert(date,Gueen) and Kunnr = :MaKhachHang and Vtweg = :MaPhuongThucBan " & _
            '        "  and  Inco1='" & g_LXLoai & "'"
            p_String = "select  Vbeln as SoHopDong, Kunnr as MaKhachHang, Inco1 from dbo.tblProjects with (nolock) where convert(date, :NgayXuat) >= " & _
                   "convert(date,Guebg) and convert(date, :NgayXuat) <= convert(date,Gueen) and Kunnr = :MaKhachHang and Vtweg = :MaPhuongThucBan " & _
                   "  and  Inco1='" & g_LXLoai & "'"
            Me.SoHopDong.SqlString = p_String
            ...
            SetItemPropertyLocal(Me, Me.Name.ToString.Trim, g_LXLoai)
        ...
        If g_LXLoai = "CIF" Then
            p_SQL = "SELECT MaNguon, TenNguon FROM tblNguon  where manguon  ='N05' and Status='X'"
            Me.MaNguon.SqlString = p_SQL

        Else
            p_SQL = "SELECT MaNguon, TenNguon FROM tblNguon  where manguon  ='N35' and Status='X'"
            Me.MaNguon.SqlString = p_SQL
        End If
            ...
            TaoColumnFind()
            Me.Text = Me.Text & " - " & g_LXLoai
            'If g_LXLoai = "CIF" Then
            '    Me.Text = Me.Text & " - CIF"
            'Else
            '    Me.Text = "Xuất bán nội địa FOB"
            'End If
    ...
    Private Sub TaoColumnFind()
        ...
        If p_GiaCongTy_PhiVT = False Then
            ...
            ' If UCase(Me.GridView1.Columns.Item("GiaCty").Name) = "GIACTY" Then
            Me.GridView1.Columns.Item("GiaCty").VisibleIndex = -1
            'End If


            ' If UCase(Me.GridView1.Columns.Item("PhiVT").Name) = "PHIVT" Then
            Me.GridView1.Columns.Item("PhiVT").VisibleIndex = -1
            'End If

            'If UCase(Me.GridView1.Columns.Item("TheM).Name) = "THEBVMT" Then
    ...
    Private Sub GridView1_CellValueChanged(ByVal sender As Object, ByVal e As DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs) Handles GridView1.CellValueChanged
        ...
        Dim p_Group As String = ""
        Dim p_Nguon, p_PTBan As String
        Dim p_Currency As String = ""
        Dim p_Row2 As DataRow
        'd()
        Dim p_DonViCCVT As String = ""
        If UCase(e.Column.FieldName) = UCase("TongDuXuat") Then
            TinhQCI()
            Exit Sub
        End If
        ...
        If UCase(e.Column.FieldName) = UCase("MaHangHoa") Or UCase(e.Column.FieldName) = UCase("DonViTinh") Or UCase(e.Column.FieldName) = UCase("PhiVT") Or UCase(e.Column.FieldName) = UCase("TheBVMT") Or UCase(e.Column.FieldName) = UCase("GiaCty") Then
            If Not e.Value Is Nothing Then
                ...
                End If
                If Not Me.PriceGroup.EditValue Is Nothing Then
                    p_Group = Me.PriceGroup.EditValue.ToString.Trim
                End If
                If p_Group = "" Then
                ...
                End If
                p_DonViCCVT = ""
                If Not Me.txtDonViCungCapVanTai.EditValue Is Nothing Then
                    p_DonViCCVT = Me.txtDonViCungCapVanTai.EditValue.ToString.Trim()
                End If
                ...
                End If
                p_NgayXuat = Me.PrcingDate.EditValue
                ...
                p_SQL = "exec FPT_LayGiaCongTy_TaiXuat '" & g_Company_Code & "' " & _
                            ...
                            ",'" & p_DVT & "' " & _
                            ",'" & p_Term & "' " & _
                            ",'" & p_NgayXuat.ToString("yyyyMMdd HH:mm:ss") & "' " & _
                            ",'" & p_LoaiTien & "' " & _
                              ",'" & p_MaTuyenDuong & "' " & _
                                ",'" & p_LHVT & "' " & _
                ...
                p_DataSet = GetDataSet(p_SQL, p_SQL) '  g_Services.Sys_SYS_GET_DATASet_Des(p_SQL, p_SQL)
                ...
                p_DataTable33 = p_DataSet.Tables(4)
                If Not p_DataTable33 Is Nothing Then
                    If p_DataTable33.Rows.Count > 0 Then
                        Try
                            Me.PriceSynDate.EditValue = p_DataTable33.Rows(0).Item(1)
                        Catch ex As Exception
    ...
    Function GetTongDonGia() As Double
        ...
        Try
            ...
            If p_Group = "06" Then
                'p_GiaCty = p_GiaCty + p_PhiVT
                GetTongDonGia = p_GiaCongTy + p_GiaVT
            Else
    ...
    Sub ProcessKeyDown()
        ...
        Dim p_CurrentDate As Date 'As DateTime

        p_GetCurrentDate(p_CurrentDate)
            ...
            If Me.SoLenh.EditValue.ToString.Trim <> "" Then
                ...
                If p_Value = "0" Then
                    ...
                    If p_Value <> Me.SoLenh.EditValue.ToString.Trim Then
                        ...
                        Me.Inco1.EditValue = g_LXLoai
                        Me.Slog.EditValue = p_Slog
                        'If Not Me.NgayXuat Is Nothing Then
                        '    Me.PrcingDate.EditValue = Me.NgayXuat.EditValue
                        'Else
                        '    Me.PrcingDate.EditValue = Now.Date
                        'End If
                        Me.PrcingDate.EditValue = p_CurrentDate
                    End If

                    p_SQL = "select dbo.GetPriceSynDate('DonGia') as SynDate"
                    p_DataTable = GetDataTable(p_SQL, p_SQL) ' g_Services.Sys_SYS_GET_DATATABLE_Des(p_SQL, p_SQL)
                    If Not p_DataTable Is Nothing Then
                        If p_DataTable.Rows.Count > 0 Then
                            'p_DataTable33 = p_DataSet.Tables(4)
                            If Not p_DataTable Is Nothing Then
                                If p_DataTable.Rows.Count > 0 Then
                                    Try
                                        Me.PriceSynDate.EditValue = p_DataTable.Rows(0).Item("SynDate")
                                    Catch ex As Exception
                ...
                Else
                    ...
                    Me.SoLenh.EditValue = p_Value
                    p_SQL = "select top 1 SoLenh , LXPhieu from tblLenhXuatE5 with (Nolock) where SoLenh='" & p_Value & "'"    '   and LXPhieu ='" & g_LXPhieu & "'"

                    If Mid(p_Value, 1, Len(g_WareHouse)) <> g_WareHouse Then
                        ShowMessageBox("", "Số lệnh không hợp lệ", 3)
                        Exit Sub
                    End If
                    p_DataTable = g_Services.Sys_SYS_GET_DATATABLE_Des(p_SQL, p_SQL)
                    If Not p_DataTable Is Nothing Then
                        If p_DataTable.Rows.Count <= 0 Then
                            ...
                            Exit Sub
                        Else
                            If p_DataTable.Rows(0).Item("LXPhieu").ToString.Trim <> g_LXPhieu Then
                                Select Case UCase(p_DataTable.Rows(0).Item("LXPhieu").ToString.Trim)
                                    Case "DICHUYEN"
                                        ShowMessageBox("", "Lệnh xuất di chuyển", 3)
                                    Case "TAIXUAT"
                                        ShowMessageBox("", "Lệnh tái xuất", 3)
                                    Case "NOIDIA"
                                        ShowMessageBox("", "Lệnh xuất nội địa", 3)
                                    Case "HANGGUI"
                                        ShowMessageBox("", "Lệnh xuất hàng gửi", 3)
                                End Select
                                Exit Sub
                            End If
                    ...
                    Else
                        ...
                        If Not p_DataTable Is Nothing Then
                            ...
                            Else
                                ...
                                If KiemTraTaoMoiLenhXuat22(p_Value, g_LXLoai, g_LXPhieu) = False Then
                                    ShowMessageBox(True, "Lệnh xuất không hợp lệ", 3)
                                    Exit Sub
                                End If
        ...
        If Me.GridView1.RowCount > 0 Then
            ...
            For p_Count = Me.GridView1.RowCount - 1 To 0 Step -1
                ...
                If Not p_DataRow Is Nothing Then
                    If p_DataRow.Item("TongXuat").ToString.Trim() = "" Or p_DataRow.Item("TongXuat").ToString.Trim() = "0" _
                             Or p_DataRow.Item("TongXuat").ToString.Trim() = "0.0" Then
                        TinhQCI()
                    End If
                End If
        ...
        If Me.GridView1.RowCount > 0 Then
            ...
            For p_Count = Me.GridView1.RowCount - 1 To 0 Step -1
                ...
                If Not p_DataRow Is Nothing Then
                    'If p_DataRow.Item("TongXuat").ToString.Trim() = "" Or p_DataRow.Item("TongXuat").ToString.Trim() = "0" _
                    '         Or p_DataRow.Item("TongXuat").ToString.Trim() = "0.0" Then
                    Double.TryParse(p_DataRow.Item("QCI_KG").ToString.Trim, p_KG)
                    Double.TryParse(p_DataRow.Item("QCI_L15").ToString.Trim, p_L15)
                    If p_KG = 0 Or p_KG = 0 Then
                        TinhQCI()
                    End If
    ...
    Sub TinhQCI()
        ...
        With Me.GridView1
            ...
            For p_Count = 0 To .RowCount - 1
                ...
                End If
                If g_tongduxuat = 0 And p_TongQuyDoi = 0 Then
                    Exit Sub
                End If
                ...
                If g_tongduxuat > 0 Then
                    ...
                    If l_quantity(0).ToString.Trim <> "" Then
                        ...
                        .SetRowCellValue(p_Count, "QCI_L15", l_quantity(1).ToString)
                        .SetRowCellValue(p_Count, "QCI_KG", l_quantity(3).ToString)
                    End If
                Else
    ...
    Sub SaveRecode()
        ...
        Me.LXPhieu.EditValue = g_LXPhieu
        Me.LXLoai.EditValue = g_LXLoai
        ...
        Dim p_DOCT As String = ""
        Dim p_SOTYPE As String = ""
        If Not Me.SOType.EditValue Is Nothing Then
            p_SOTYPE = Me.SOType.EditValue.ToString.Trim
        End If
        If p_SOTYPE = "ZIS" Then
            If Not Me.DOCT.EditValue Is Nothing Then
                p_DOCT = Me.DOCT.EditValue.ToString.Trim
            End If
            If p_DOCT = "" Then
                ShowMessageBox("", "DO CT chưa được nhập")
                Exit Sub
            End If
        End If

        'anhqh20200424  kiem tra khong cho nhap nhieu loai tien
        For p_Count = 0 To Me.GridView1.RowCount - 1
            p_DataRow = GridView1.GetDataRow(p_Count)
            If Not p_DataRow Is Nothing Then
                If p_DataRow.Item("CurrencyKey").ToString.Trim <> "" Then
                    If p_String = "" Then
                        p_String = p_DataRow.Item("CurrencyKey").ToString.Trim
                    Else
                        If p_String <> p_DataRow.Item("CurrencyKey").ToString.Trim Then
                            ShowMessageBox("", "Lệnh xuất có nhiều loại tiền")
                            Exit Sub
                        End If
                    End If
                End If
    ...
    Private Sub ToolStripButton1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ToolStripButton1.Click
        ...
        Dim p_Tmp As String
        p_Tmp = ""
        If Not Me.LXLoai.EditValue Is Nothing Then
            p_Tmp = Me.LXLoai.EditValue.ToString.Trim
        End If
        If p_Tmp = "" Then
            Me.LXLoai.EditValue = g_LXLoai
        End If
        p_Tmp = ""
        If Not Me.LXPhieu.EditValue Is Nothing Then
            p_Tmp = Me.LXPhieu.EditValue.ToString.Trim
        End If
        If p_Tmp = "" Then
            Me.LXPhieu.EditValue = g_LXPhieu
        End If
        ...
        If Me.FormStatus = True Then
            ...
            If KiemTraTonKhoToKhai() = False Then
                Exit Sub
            End If
    ...
    Private Sub ToolStripButton3_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ToolStripButton3.Click
        ...
        Dim FrmLenhXuatAdd As New FrmLenhXuatCopy
        ...
        FrmLenhXuatAdd.p_LenhXuatLoai = g_LXLoai
        FrmLenhXuatAdd.p_LenhXuatPhieu = g_LXPhieu
        FrmLenhXuatAdd.ShowDialog(Me)

        p_SoLenh = FrmLenhXuatAdd.g_SoLenhCopy
        If p_SoLenh <> "" Then
            'If InStr(p_SoLenh, ",", CompareMethod.Text) = 0 Then
            '    ToolStripButton2_Click(sender, Nothing)
            '    Me.SoLenh.EditValue = p_SoLenh
            '    ProcessKeyDown()
            ...
            '    Me.SoLenh.Focus()
            'End If
            ShowMessageBox("", "Danh sách lệnh xuất mới (" & p_SoLenh & ")", 1)
    ...
    Private Sub ResetPrice()
        ...
        For p_Count = 0 To GridView1.RowCount - 1
            ...
            If Not Me.PrcingDate.EditValue Is Nothing Then
                p_PricingDate = CDate(Me.PrcingDate.EditValue).ToString("yyyyMMdd HH:mm:ss")
            End If
            p_SQL = ""

            If p_PricingDate = "" Then
                ShowMessageBox("", "Pricing date chưa nhập")
                Exit Sub
            End If
            p_SQL = "exec FPT_LayGiaCongTy_TaiXuat '" & g_Company_Code & "' " & _
                         ",'" & p_PTBan & "' " & _
    ...
    Private Sub PrcingDate_EditValueChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles PrcingDate.EditValueChanged
        ...
        If p_Date.IsModified = True Then
            ResetPrice()
            'Else
            '    If p_Date.OldEditValue Is Nothing Then
            '        ResetPrice()
            '    ElseIf p_Date.EditValue <> p_Date.EditValue Then
            '        ResetPrice()
            '    End If
        End If
    End Sub
    ...
    Private Sub SoHopDong_EditValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles SoHopDong.EditValueChanged
        Dim p_ButtonEdit As New U_TextBox.U_ButtonEdit
        p_ButtonEdit = CType(sender, U_TextBox.U_ButtonEdit)

        If p_ButtonEdit.IsModified Then
            Dim p_SQL As String = ""
            Dim p_Datata As DataTable
            Dim p_SoHD As String = ""
            If Not Me.SoHopDong.EditValue Is Nothing Then
                p_SoHD = Me.SoHopDong.EditValue.ToString.Trim
            End If
            p_SQL = " select  convert(date,case when isdate(Guebg) =0 then null else  Guebg end) as NgayHopDong  from dbo.tblProjects with (nolock)  where Vbeln ='" & p_SoHD & "' and isdate(Guebg) =1  "
            p_Datata = GetDataTable(p_SQL, p_SQL)
            If Not p_Datata Is Nothing Then
                If p_Datata.Rows.Count > 0 Then
                    Me.NgayHopDong.EditValue = CDate(p_Datata.Rows(0).Item(0))
                End If
            End If
    ...
    Private Sub ToolStripButton4_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ToolStripButton4.Click
        ...
        Dim p_SoLenh As String = ""
        Dim p_Exit As Boolean = False
        Dim p_Bindsource As U_TextBox.U_BindingSource
        ' Dim p_Datatable As DataTable
        If Me.FormStatus = True Then
            ShowMessageBox("", "Lênh xuất chưa được lưu")
            Exit Sub
        End If
        ...
        If Not Me.SoLenh.EditValue Is Nothing Then
            p_SoLenh = Me.SoLenh.EditValue.ToString.Trim
        End If
        If Microsoft.VisualBasic.Strings.Left(p_SoLenh, Len(g_WareHouse)) = g_WareHouse Then
            p_SQL = "select SoLenh from tblLenhXuatE5  with (nolock)  where SoLenh ='" & p_SoLenh & "'"
            p_Datatable = GetDataTable(p_SQL, p_SQL)
            If Not p_Datatable Is Nothing Then
                If p_Datatable.Rows.Count > 0 Then
                    p_Exit = True
        ...
        End If
        If p_Exit = False Then
            ShowMessageBox("", "Lệnh không hơp lệ")
            Exit Sub
        End If

        p_Bindsource = Me.TrueDBGrid1.DataSource
        ...
        Dim FrmDO1 As New FrmLenhXuat_TaiXuatDO1
        FrmDO1.p_XtraModuleObj = p_XtraModuleObj ' p_FptModule
        FrmDO1.g_XtraServicesObj = g_XtraServicesObj
        FrmDO1.SoDO2 = p_SoLenh
        'FrmMenu.DefaultWhere = "DocEntry=" & p_DataRow.Item(Me.U_TrueDBGrid1.ColumnKey).ToString.Trim
        FrmDO1.p_XtraDataSet_TrueGird = p_DataSet_TrueGird
        ' FrmLenhXuatAdd.READ_ONLY = Me.READ_ONLY
        FrmDO1.p_DataLineHH = CType(p_Bindsource.DataSource, DataTable)
        FrmDO1.p_XtraToolTripLabel = g_ToolStripStatus
        ' FrmATG.p_XtraDataSet_TrueGird = p_DataSet_TrueGird
        FrmDO1.p_XtraMessageStatusl = g_MessageStatus
        FrmDO1.p_PhuongThucBan = p_PTBan
        FrmDO1.ShowDialog(Me)

    End Sub

    'Private Sub DiemTraHang_EditValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles DiemTraHang.EditValueChanged

    'End Sub

    'Private Sub DiemTraHang_MouseMove(ByVal sender As Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles DiemTraHang.MouseMove
    '    Me.DiemTraHang.ToolTip = Me.DiemTraHang.Text
    'End Sub
    ...
    Private Sub PaymentMethod_Validating(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles _
                             U_ButtonEdit3.Validating, MaKhachHang.Validating, Term.Validating, _
                             PriceGroup.Validating, MaVanChuyen.Validating, txtDonViCungCapVanTai.Validating
    ...
    Private Sub SoTKHQNhap_EditValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles SoTKHQNhap.EditValueChanged

    End Sub

    Private Sub SoTKHQNhap_Validating(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles SoTKHQNhap.Validating
        Dim p_ButtoEdit As U_TextBox.U_ButtonEdit
        p_ButtoEdit = CType(sender, U_TextBox.U_ButtonEdit)
        If p_ButtoEdit.IsModified = True Then
            SoTonToKhai()
        End If
    End Sub

    Private Sub XtraTabPage5_Paint(ByVal sender As System.Object, ByVal e As System.Windows.Forms.PaintEventArgs) Handles XtraTabPage5.Paint
