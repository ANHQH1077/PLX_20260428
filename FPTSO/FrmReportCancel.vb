...
Public Class FrmReportCancel
    Private p_BindingSource As New U_TextBox.U_BindingSource
    Private p_DataTable As DataTable

    Private p_Int As Integer = 1000
    Private p_StrColumn As String = "select distinct  SoBienban , convert(nvarchar (10),CreateDate, 103) as CreateDate, SoLenh " &
                                " from tblBienBanGiaoNhan_Hist  h  with (Nolock) where sStatus ='I' " &
                                " And Convert( Date, CreateDate) >= Convert( Date, getdate() - " & p_Int & ") " &
                                " and not exists  (select 1 from ( " &
                                " Select SoBienban from tblBienBanGiaoNhan_Hist  h with (nolock) where sStatus ='C' " &
                                " And convert(date,cancelDate) >=convert(date,getdate() - " & p_Int & ") ) aa  where h.SoBienban =aa.SoBienban ) " &
                                " and charindex(Client,(select Terminal from sys_user  where upper( User_Name ) =upper('" & g_UserName & "')	),1) >0 " & _
                                ""
