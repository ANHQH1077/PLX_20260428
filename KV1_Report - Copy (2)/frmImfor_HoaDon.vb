    ...
    Private Function Alter_Sub_Table(ByVal dtTable0 As DataTable, ByVal dtTable As DataTable) As DataTable
        ...
        Try
            ...
            For Each r As DataRow In dtTable.Rows
                ...
                'anhqh
                '20170718
                ' làm tròn theo từng dòng hàng hóa
                W_ThueGTGT = W_ThueGTGT + Math.Round(((l_thanhtien - CInt(p_DataRow(5).ToString().Trim())) * Convert.ToInt32(IIf(txtThueGTGT.Text.ToString.Trim = "", 0, txtThueGTGT.EditValue))) / 100, 0, MidpointRounding.AwayFromZero)

            Next

            'anhqh
            '20170718
            'Tam bo di tính công thức và làm tròn theo từng dòng hàng hóa
            ' W_ThueGTGT = Math.Round(((TongTien - W_ChietKhau) * Convert.ToInt32(IIf(txtThueGTGT.Text.ToString.Trim = "", 0, txtThueGTGT.EditValue))) / 100, 0, MidpointRounding.AwayFromZero)
        Catch ex As Exception
            Return Nothing
        End Try
        Return SubTable
